...
val androidStoreFile = providers.gradleProperty("MUSTB_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("MUSTB_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("MUSTB_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("MUSTB_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set MUSTB_ANDROID_STORE_FILE, " +
            "MUSTB_ANDROID_STORE_PASSWORD, MUSTB_ANDROID_KEY_ALIAS, and " +
            "MUSTB_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/Must-bKit/Sources/Must-bKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.must-b.app"
        minSdk = 31
        targetSdk = 36
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName = "must-b-$versionName-$buildType.apk"
