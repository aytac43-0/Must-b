...
private const val NOTIFICATION_CHANNEL_BASE_ID = "must-b.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Must-b Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Must-b Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Must-b Active")
      }
