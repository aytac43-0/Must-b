...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.must-b.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Must-b", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Must-b gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Must-b.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "Must-bShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Must-b is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Must-b Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("must-b-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
