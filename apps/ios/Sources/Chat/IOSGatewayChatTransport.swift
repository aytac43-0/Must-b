...
struct IOSGatewayChatTransport: Must-bChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> Must-bChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(Must-bChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> Must-bChatHistoryPayload {
        ...
        return try JSONDecoder().decode(Must-bChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [Must-bChatAttachmentPayload]) async throws -> Must-bChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [Must-bChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(Must-bChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(Must-bGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: Must-bGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: Must-bChatEventPayload.self)
                        {
