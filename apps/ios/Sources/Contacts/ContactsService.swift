...
final class ContactsService: ContactsServicing {
    ...
    func search(params: Must-bContactsSearchParams) async throws -> Must-bContactsSearchPayload {
        ...
        return Must-bContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: Must-bContactsAddParams) async throws -> Must-bContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return Must-bContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return Must-bContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> Must-bContactPayload {
        Must-bContactPayload(
