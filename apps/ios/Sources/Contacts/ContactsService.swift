...
final class ContactsService: ContactsServicing {
    ...
    func search(params: MustBContactsSearchParams) async throws -> MustBContactsSearchPayload {
        ...
        return MustBContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: MustBContactsAddParams) async throws -> MustBContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return MustBContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return MustBContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> MustBContactPayload {
        MustBContactPayload(
