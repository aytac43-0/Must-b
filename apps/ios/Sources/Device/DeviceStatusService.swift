...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> Must-bDeviceStatusPayload {
        ...
        return Must-bDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> Must-bDeviceInfoPayload {
        ...
        return Must-bDeviceInfoPayload(
    ...
    private func batteryStatus() -> Must-bBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: Must-bBatteryState = switch device.batteryState {
        ...
        return Must-bBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> Must-bThermalStatusPayload {
        let state: Must-bThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return Must-bThermalStatusPayload(state: state)
    }

    private func storageStatus() -> Must-bStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return Must-bStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
