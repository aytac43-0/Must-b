...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> MustBDeviceStatusPayload {
        ...
        return MustBDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    private func batteryStatus() -> MustBBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: MustBBatteryState = switch device.batteryState {
        ...
        return MustBBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> MustBThermalStatusPayload {
        let state: MustBThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return MustBThermalStatusPayload(state: state)
    }

    private func storageStatus() -> MustBStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return MustBStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
