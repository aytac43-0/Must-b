...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [MustBCapability.canvas.rawValue, MustBCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(MustBCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(MustBCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = MustBLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(MustBCapability.location.rawValue) }

        caps.append(MustBCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(MustBCapability.watch.rawValue)
        }
        caps.append(MustBCapability.photos.rawValue)
        caps.append(MustBCapability.contacts.rawValue)
        caps.append(MustBCapability.calendar.rawValue)
        caps.append(MustBCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(MustBCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            MustBScreenCommand.record.rawValue,
            MustBSystemCommand.notify.rawValue,
            MustBChatCommand.push.rawValue,
        ...
        if caps.contains(MustBCapability.camera.rawValue) {
            commands.append(MustBCameraCommand.list.rawValue)
            commands.append(MustBCameraCommand.snap.rawValue)
            commands.append(MustBCameraCommand.clip.rawValue)
        }
        if caps.contains(MustBCapability.location.rawValue) {
            commands.append(MustBLocationCommand.get.rawValue)
        }
        if caps.contains(MustBCapability.device.rawValue) {
            commands.append(MustBDeviceCommand.status.rawValue)
            commands.append(MustBDeviceCommand.info.rawValue)
        }
        if caps.contains(MustBCapability.watch.rawValue) {
            commands.append(MustBWatchCommand.status.rawValue)
            commands.append(MustBWatchCommand.notify.rawValue)
        }
        if caps.contains(MustBCapability.photos.rawValue) {
            commands.append(MustBPhotosCommand.latest.rawValue)
        }
        if caps.contains(MustBCapability.contacts.rawValue) {
            commands.append(MustBContactsCommand.search.rawValue)
            commands.append(MustBContactsCommand.add.rawValue)
        }
        if caps.contains(MustBCapability.calendar.rawValue) {
            commands.append(MustBCalendarCommand.events.rawValue)
            commands.append(MustBCalendarCommand.add.rawValue)
        }
        if caps.contains(MustBCapability.reminders.rawValue) {
            commands.append(MustBRemindersCommand.list.rawValue)
            commands.append(MustBRemindersCommand.add.rawValue)
        }
        if caps.contains(MustBCapability.motion.rawValue) {
            commands.append(MustBMotionCommand.activity.rawValue)
            commands.append(MustBMotionCommand.pedometer.rawValue)
        }
