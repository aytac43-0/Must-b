...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [Must-bCapability.canvas.rawValue, Must-bCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(Must-bCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(Must-bCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = Must-bLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(Must-bCapability.location.rawValue) }

        caps.append(Must-bCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(Must-bCapability.watch.rawValue)
        }
        caps.append(Must-bCapability.photos.rawValue)
        caps.append(Must-bCapability.contacts.rawValue)
        caps.append(Must-bCapability.calendar.rawValue)
        caps.append(Must-bCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(Must-bCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            Must-bScreenCommand.record.rawValue,
            Must-bSystemCommand.notify.rawValue,
            Must-bChatCommand.push.rawValue,
        ...
        if caps.contains(Must-bCapability.camera.rawValue) {
            commands.append(Must-bCameraCommand.list.rawValue)
            commands.append(Must-bCameraCommand.snap.rawValue)
            commands.append(Must-bCameraCommand.clip.rawValue)
        }
        if caps.contains(Must-bCapability.location.rawValue) {
            commands.append(Must-bLocationCommand.get.rawValue)
        }
        if caps.contains(Must-bCapability.device.rawValue) {
            commands.append(Must-bDeviceCommand.status.rawValue)
            commands.append(Must-bDeviceCommand.info.rawValue)
        }
        if caps.contains(Must-bCapability.watch.rawValue) {
            commands.append(Must-bWatchCommand.status.rawValue)
            commands.append(Must-bWatchCommand.notify.rawValue)
        }
        if caps.contains(Must-bCapability.photos.rawValue) {
            commands.append(Must-bPhotosCommand.latest.rawValue)
        }
        if caps.contains(Must-bCapability.contacts.rawValue) {
            commands.append(Must-bContactsCommand.search.rawValue)
            commands.append(Must-bContactsCommand.add.rawValue)
        }
        if caps.contains(Must-bCapability.calendar.rawValue) {
            commands.append(Must-bCalendarCommand.events.rawValue)
            commands.append(Must-bCalendarCommand.add.rawValue)
        }
        if caps.contains(Must-bCapability.reminders.rawValue) {
            commands.append(Must-bRemindersCommand.list.rawValue)
            commands.append(Must-bRemindersCommand.add.rawValue)
        }
        if caps.contains(Must-bCapability.motion.rawValue) {
            commands.append(Must-bMotionCommand.activity.rawValue)
            commands.append(Must-bMotionCommand.pedometer.rawValue)
        }
