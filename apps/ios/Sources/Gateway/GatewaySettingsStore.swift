...
enum GatewaySettingsStore {
    private static let gatewayService = "ai.must-b.gateway"
    private static let nodeService = "ai.must-b.node"
    private static let talkService = "ai.must-b.talk"
...
enum GatewayDiagnostics {
    private static let logger = Logger(subsystem: "ai.must-b.ios", category: "GatewayDiag")
    private static let queue = DispatchQueue(label: "ai.must-b.gateway.diagnostics")
    ...
    private static var fileURL: URL? {
        FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first?
            .appendingPathComponent("must-b-gateway.log")
    }
