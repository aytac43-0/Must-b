...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: Must-bWatchNotifyParams) -> Must-bWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [Must-bWatchAction]?,
        kind: String?,
        promptId: String?) -> [Must-bWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> Must-bWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return Must-bWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                Must-bWatchAction(id: "approve", label: "Approve"),
                Must-bWatchAction(id: "decline", label: "Decline", style: "destructive"),
                Must-bWatchAction(id: "open_phone", label: "Open iPhone"),
                Must-bWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            Must-bWatchAction(id: "done", label: "Done"),
            Must-bWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            Must-bWatchAction(id: "open_phone", label: "Open iPhone"),
            Must-bWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: Must-bWatchRisk?,
        priority: Must-bNotificationPriority?) -> Must-bWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: Must-bNotificationPriority?,
        risk: Must-bWatchRisk?) -> Must-bNotificationPriority?
    {
