...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: MustBWatchNotifyParams) -> MustBWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [MustBWatchAction]?,
        kind: String?,
        promptId: String?) -> [MustBWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> MustBWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return MustBWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                MustBWatchAction(id: "approve", label: "Approve"),
                MustBWatchAction(id: "decline", label: "Decline", style: "destructive"),
                MustBWatchAction(id: "open_phone", label: "Open iPhone"),
                MustBWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            MustBWatchAction(id: "done", label: "Done"),
            MustBWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            MustBWatchAction(id: "open_phone", label: "Open iPhone"),
            MustBWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: MustBWatchRisk?,
        priority: MustBNotificationPriority?) -> MustBWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: MustBNotificationPriority?,
        risk: MustBWatchRisk?) -> MustBNotificationPriority?
    {
