...
final class MotionService: MotionServicing {
    func activities(params: Must-bMotionActivityParams) async throws -> Must-bMotionActivityPayload {
        ...
        let mapped: [Must-bMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return Must-bMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: Must-bPedometerParams) async throws -> Must-bPedometerPayload {
        ...
        let payload: Must-bPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = Must-bPedometerPayload(
