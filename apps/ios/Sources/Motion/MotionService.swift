...
final class MotionService: MotionServicing {
    func activities(params: MustBMotionActivityParams) async throws -> MustBMotionActivityPayload {
        ...
        let mapped: [MustBMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return MustBMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: MustBPedometerParams) async throws -> MustBPedometerPayload {
        ...
        let payload: MustBPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = MustBPedometerPayload(
