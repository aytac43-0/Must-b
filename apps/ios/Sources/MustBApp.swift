...
@MainActor
final class Must-bAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.must-b.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.must-b.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.must-b.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "must-b.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "must-b.watch.promptId"
    static let sessionKeyKey = "must-b.watch.sessionKey"
    static let actionPrimaryIDKey = "must-b.watch.action.primary.id"
    static let actionPrimaryLabelKey = "must-b.watch.action.primary.label"
    static let actionSecondaryIDKey = "must-b.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "must-b.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "must-b.watch.action.primary"
    static let actionSecondaryIdentifier = "must-b.watch.action.secondary"
    static let actionIdentifierPrefix = "must-b.watch.action."
    static let actionIDKeyPrefix = "must-b.watch.action.id."
    static let actionLabelKeyPrefix = "must-b.watch.action.label."
    static let categoryPrefix = "must-b.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: Must-bWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> Must-bWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return Must-bWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Must-b" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [Must-bWatchAction]) -> [UNNotificationAction] {
...
@main
struct Must-bApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(Must-bAppDelegate.self) private var appDelegate
...
extension Must-bApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Must-b: installing uncaught exception handler")
