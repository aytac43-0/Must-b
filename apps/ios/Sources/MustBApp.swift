...
@MainActor
final class MustBAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
...
enum WatchPromptNotificationBridge {
        ...
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> MustBWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return MustBWatchAction(id: id, label: label, style: action.style)
        }
    ...
    private static func categoryActions(_ actions: [MustBWatchAction]) -> [UNNotificationAction] {
...
struct MustBApp: App {
    ...
    @UIApplicationDelegateAdaptor(MustBAppDelegate.self) private var appDelegate
