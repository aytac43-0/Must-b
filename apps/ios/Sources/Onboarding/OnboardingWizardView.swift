...
struct OnboardingWizardView: View {
    ...
    private var welcomeStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Connect to your Must-b gateway")
                .font(.subheadline)
                .foregroundStyle(.secondary)
            ...
            if self.issue.needsPairing {
                Section {
                    ...
                    let requestLine: String = {
                        ...
                        return "Request ID: check `must-b devices list`."
                    }()
                    Text(
                        "Approve this device on the gateway.\n"
                            + "1) `must-b devices approve` (or `must-b devices approve <requestId>`)\n"
                            + "2) `/pair approve` in your Must-b chat\n"
                            + "\(requestLine)\n"
                            + "Must-b will also retry automatically when you return to this app.")
                }
            }
    ...
    private func initializeState() {
        ...
        if self.selectedMode == .developerLocal && self.manualHost == "must-b.local" {
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "must-b.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "must-b.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "must-b.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:
