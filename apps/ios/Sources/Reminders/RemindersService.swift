...
final class RemindersService: RemindersServicing {
    func list(params: MustBRemindersListParams) async throws -> MustBRemindersListPayload {
        ...
        let payload: [MustBReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return MustBRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: MustBRemindersAddParams) async throws -> MustBRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = MustBReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return MustBRemindersAddPayload(reminder: payload)
    }
