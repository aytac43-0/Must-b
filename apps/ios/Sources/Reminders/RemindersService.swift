...
final class RemindersService: RemindersServicing {
    func list(params: Must-bRemindersListParams) async throws -> Must-bRemindersListPayload {
        ...
        let payload: [Must-bReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return Must-bRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: Must-bRemindersAddParams) async throws -> Must-bRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = Must-bReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return Must-bRemindersAddPayload(reminder: payload)
    }
