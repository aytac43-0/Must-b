...
struct SettingsTab: View {
    ...
    @AppStorage("location.enabledMode") private var locationEnabledModeRaw: String = Must-bLocationMode.off.rawValue
    ...
    @State private var lastLocationModeRaw: String = Must-bLocationMode.off.rawValue
    ...
    private let gatewayLogger = Logger(subsystem: "ai.must-b.ios", category: "GatewaySettings")
    ...
    var body: some View {
        NavigationStack {
                ...
                Section {
                    DisclosureGroup(isExpanded: self.$gatewayExpanded) {
                        if !self.isGatewayConnected {
                            Text(
                                "1. Open a chat with your Must-b agent and send /pair\n"
                ...
                Section("Device") {
                    DisclosureGroup("Features") {
                        ...
                        self.featureToggle(
                            ...
                            help: "Enables voice conversation mode with your connected Must-b agent.") { newValue in
                        ...
                        HStack(spacing: 8) {
                            ...
                            Button {
                                self.activeFeatureHelp = FeatureHelp(
                                    ...
                                    message: "Controls location permissions for Must-b. "
                        ...
                        Picker("Location Access", selection: self.$locationEnabledModeRaw) {
                            Text("Off").tag(Must-bLocationMode.off.rawValue)
                            Text("While Using").tag(Must-bLocationMode.whileUsing.rawValue)
                            Text("Always").tag(Must-bLocationMode.always.rawValue)
                        }
                        .labelsHidden()
                        ...
                        self.featureToggle(
                            ...
                            help: "Keeps the screen awake while Must-b is open.")
                    ...
                    DisclosureGroup("Device Info") {
                        ...
                        LabeledContent("Must-b", value: DeviceInfoHelper.mustBVersionString())
                    }
                }
            }
            ...
            .onChange(of: self.locationEnabledModeRaw) { _, newValue in
                ...
                guard let mode = Must-bLocationMode(rawValue: newValue) else { return }
                Task {
    ...
    private func friendlyGatewayMessage(from raw: String) -> String? {
        ...
        if lower.contains("pairing required") {
            return "Pairing required. Go back to your Must-b chat and run /pair approve, then tap Connect again."
        }
