...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__MUSTB_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<Must-bTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.must-b", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = Must-bTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> Must-bTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return Must-bTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> Must-bTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> Must-bTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return Must-bTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> Must-bTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = Must-bTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: Must-bTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: Must-bAgentEventPayload.self
            ) else {
                continue
            }
