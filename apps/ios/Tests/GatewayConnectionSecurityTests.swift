...
@Suite(.serialized) struct GatewayConnectionSecurityTests {
    ...
    {
        let endpoint: NWEndpoint = .service(name: "Test", type: "_must-b-gw._tcp", domain: "local.", interface: nil)
    ...
    private func clearTLSFingerprint(stableID: String) {
        let suite = UserDefaults(suiteName: "ai.must-b.shared") ?? .standard
    ...
    @Test @MainActor func manualConnectionsForceTLSForNonLoopbackHosts() async {
        ...
        #expect(controller._test_resolveManualUseTLS(host: "must-b.local", useTLS: false) == true)
