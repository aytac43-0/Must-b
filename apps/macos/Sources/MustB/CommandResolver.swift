...
enum CommandResolver {
    private static let projectRootDefaultsKey = "must-b.gatewayProjectRootPath"
    private static let helperName = "must-b"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let must-bEntry = root.appendingPathComponent("must-b.mjs").path
        if FileManager().isReadableFile(atPath: must-bEntry) { return must-bEntry }
        let binEntry = root.appendingPathComponent("bin/must-b.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/must-b")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let must-bPaths = self.must-bManagedPaths(home: home)
        if !must-bPaths.isEmpty {
            extras.insert(contentsOf: must-bPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + must-bPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func must-bManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".must-b"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func must-bExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectMust-bExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("must-b.mjs").path,
            root.appendingPathComponent("bin/must-b.js").path,
        ]
    ...
    static func hasAnyMust-bInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.must-bExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let must-bPath = self.projectMust-bExecutable(projectRoot: root) {
            return [must-bPath, subcommand] + extraArgs
        }
        if let must-bPath = self.must-bExecutable(searchPaths: searchPaths) {
            return [must-bPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "must-b", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            must-b entrypoint missing (looked for dist/index.js or must-b.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real must-b CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/must-b"
        ...
        \(cliSection)
        \(projectSection)
        if command -v must-b >/dev/null 2>&1; then
          CLI="$(command -v must-b)"
          must-b \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/must-b.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/must-b.mjs"
            node "$PRJ/must-b.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/must-b.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/must-b.js"
            node "$PRJ/bin/must-b.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent must-b"
          pnpm --silent must-b \(quotedArgs);
        else
          echo "must-b CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? Must-bConfigFile.loadDict()
