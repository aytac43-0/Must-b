...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: MustBProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: MustBKit.AnyCodable]()) {
                $0[$1.key] = MustBKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: MustBProtocol.AnyCodable?) -> [String: MustBProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: MustBProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: MustBKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: MustBProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
