...
enum DebugActions {
    private static let verboseDefaultsKey = "must-b.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "must-b.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = Must-bPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Must-b", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = Must-bPaths.configURL
        guard
