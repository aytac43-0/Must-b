...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["MUSTB_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/must-b")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("must-b-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("must-b-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/must-b/ (rolling or stdout), or nil if none exist.
