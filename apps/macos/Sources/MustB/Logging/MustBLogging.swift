...
enum MustBLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = MustBOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = MustBFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        MustBLogging.bootstrapIfNeeded()
        let label = MustBLogging.makeLabel(subsystem: subsystem, category: category)
...
struct MustBOSLogHandler: AppLogLevelBackedHandler {
...
struct MustBFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = MustBLogging.parseLabel(self.label)
