...
enum Must-bLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = Must-bOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = Must-bFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        Must-bLogging.bootstrapIfNeeded()
        let label = Must-bLogging.makeLabel(subsystem: subsystem, category: category)
...
struct Must-bOSLogHandler: AppLogLevelBackedHandler {
...
struct Must-bFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = Must-bLogging.parseLabel(self.label)
