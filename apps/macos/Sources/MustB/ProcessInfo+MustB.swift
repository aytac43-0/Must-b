...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.must-b.mac`) even if the shipped
    ...
    {
        if environment["MUSTB_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "must-b.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "must-b.nixMode") { return true }
