...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> MustBSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: MustBSessionPreviewEntry,
    ...
    private static func cache(payload: MustBSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: MustBChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [MustBKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [MustBKit.AnyCodable]) -> [MustBChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(MustBChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: MustBChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: MustBChatMessage) -> Bool {
    ...
    private static func toolNames(for message: MustBChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: MustBChatMessage) -> String? {
