...
struct SettingsRootView: View {
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .must-bSelectSettingsTab)) { note in
    ...
    private var nixManagedBanner: some View {
        ...
        let configPath = self.snapshotPaths.configPath ?? Must-bPaths.configURL.path
        let stateDir = self.snapshotPaths.stateDir ?? Must-bPaths.stateDirURL.path
...
extension Notification.Name {
    static let must-bSelectSettingsTab = Notification.Name("must-bSelectSettingsTab")
}
