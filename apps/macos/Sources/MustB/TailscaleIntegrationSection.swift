...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.must-b/must-b.json. Prefer MUSTB_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.must-b/must-b.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.must-b/must-b.json. Restart the gateway to apply."
        }
