...
actor TalkModeRuntime {
    ...
    private let logger = Logger(subsystem: "ai.must-b", category: "talk.runtime")
    private let ttsLogger = Logger(subsystem: "ai.must-b", category: "talk.tts")
    ...
    private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
        do {
            ...
            let decoded: [Must-bChatMessage] = messages.compactMap { item in
