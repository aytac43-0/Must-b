...
struct MacGatewayChatTransport: MustBChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> MustBChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(MustBChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            MustBChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return MustBChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> MustBChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                MustBGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: MustBProtocol.ModelChoice) -> MustBChatModelChoice {
        MustBChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<MustBChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any MustBChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = MustBChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: MustBChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<MustBChatView>) -> NSViewController
    {
