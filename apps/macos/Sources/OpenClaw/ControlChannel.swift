...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: Must-bProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.must-b", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: Must-bKit.AnyCodable]()) {
                $0[$1.key] = Must-bKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: Must-bProtocol.AnyCodable?) -> [String: Must-bProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: Must-bProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: Must-bKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: Must-bProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("must-b.control.heartbeat")
    static let controlAgentEvent = Notification.Name("must-b.control.agent")
}
