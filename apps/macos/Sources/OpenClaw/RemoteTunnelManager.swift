...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.must-b", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Must-b run already has an SSH listener on the expected port (common after restarts),
