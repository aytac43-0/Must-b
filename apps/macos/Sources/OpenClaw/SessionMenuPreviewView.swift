...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.must-b", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> Must-bSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: Must-bSessionPreviewEntry,
    ...
    private static func cache(payload: Must-bSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: Must-bChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [Must-bKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [Must-bKit.AnyCodable]) -> [Must-bChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(Must-bChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: Must-bChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: Must-bChatMessage) -> Bool {
    ...
    private static func toolNames(for message: Must-bChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: Must-bChatMessage) -> String? {
