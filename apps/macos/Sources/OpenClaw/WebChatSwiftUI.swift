...
private let webChatSwiftLogger = Logger(subsystem: "ai.must-b", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "must-b.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: Must-bChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> Must-bChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(Must-bChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            Must-bChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return Must-bChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> Must-bChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                Must-bGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: Must-bProtocol.ModelChoice) -> Must-bChatModelChoice {
        Must-bChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<Must-bChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any Must-bChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = Must-bChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: Must-bChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<Must-bChatView>) -> NSViewController
    {
