...
struct Must-bMacCLI {
    static func main() async {
        ...
        default:
            fputs("must-b-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      must-b-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      must-b-mac discover [--timeout <ms>] [--json] [--include-local]
      must-b-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      must-b-mac connect
      must-b-mac connect --url ws://127.0.0.1:18789 --json
      must-b-mac discover --timeout 3000 --json
      must-b-mac wizard --mode local
    """)
}
