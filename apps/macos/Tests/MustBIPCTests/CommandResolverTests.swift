...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `prefers must-b binary`() throws {
        let defaults = self.makeLocalDefaults()

        let tmp = try makeTempDirForTests()
        CommandResolver.setProjectRoot(tmp.path)

        let must-bPath = tmp.appendingPathComponent("node_modules/.bin/must-b")
        try makeExecutableForTests(at: must-bPath)

        let cmd = CommandResolver.must-bCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([must-bPath.path, "gateway"]))
    }
    ...
    @Test func `prefers must-b binary over pnpm`() throws {
        let defaults = self.makeLocalDefaults()
        ...
        let must-bPath = binDir.appendingPathComponent("must-b")
        ...
        #expect(cmd.prefix(2).elementsEqual([must-bPath.path, "rpc"]))
    }

    @Test func `uses must-b binary without node runtime`() throws {
        let defaults = self.makeLocalDefaults()
    ...
    @Test func `falls back to pnpm`() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "must-b", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "must-b", "health", "--json"]))
    ...
    @Test func `builds SSH command for remote mode`() {
        ...
        defaults.set("must-b@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/must-b", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "must-b@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/must-b'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("must-b"))
    ...
    @Test func `config root local overrides remote defaults`() throws {
        ...
        #expect(cmd.first == must-bPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
