...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: MustBProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: MustBProtocol.AnyCodable] = [
            "presence": .init([MustBProtocol.AnyCodable(entry)]),
        ]
        let payload = MustBProtocol.AnyCodable(payloadMap)
