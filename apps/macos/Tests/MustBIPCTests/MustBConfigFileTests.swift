...
struct MustBConfigFileTests {
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["MUSTB_CONFIG_PATH": override]) {
            #expect(MustBConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["MUSTB_CONFIG_PATH": override]) {
            ...
            #expect(MustBConfigFile.remoteGatewayPort() == 19999)
            #expect(MustBConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(MustBConfigFile.remoteGatewayPort(matchingHost: "gateway") == 19999)
            #expect(MustBConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
        }
    }
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["MUSTB_CONFIG_PATH": override]) {
            ...
            MustBConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["MUSTB_CONFIG_PATH": override]) {
            ...
            MustBConfigFile.clearRemoteGatewayUrl()
            let root = MustBConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        ...
        await TestIsolation.withEnvValues([
            ...
            #expect(MustBConfigFile.stateDirURL().path == dir)
            #expect(MustBConfigFile.url().path == "\(dir)/must-b.json")
        }
    }
