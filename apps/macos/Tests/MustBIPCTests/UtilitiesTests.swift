...
@Suite(.serialized) struct UtilitiesTests {
    ...
    @Test func `gateway entrypoint prefers dist over bin`() throws {
        ...
        let bin = tmp.appendingPathComponent("bin/must-b.js")
    ...
    @Test func `log locator picks newest log file`() throws {
        let fm = FileManager()
        let dir = URL(fileURLWithPath: "/tmp/must-b", isDirectory: true)
        try? fm.createDirectory(at: dir, withIntermediateDirectories: true)

        let older = dir.appendingPathComponent("must-b-old-\(UUID().uuidString).log")
        let newer = dir.appendingPathComponent("must-b-new-\(UUID().uuidString).log")
