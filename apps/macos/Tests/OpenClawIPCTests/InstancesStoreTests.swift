...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: Must-bProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: Must-bProtocol.AnyCodable] = [
            "presence": .init([Must-bProtocol.AnyCodable(entry)]),
        ]
        let payload = Must-bProtocol.AnyCodable(payloadMap)
