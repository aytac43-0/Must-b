...
struct ChatMessageBubble: View {
    let message: MustBChatMessage
    let style: MustBChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? MustBChatTheme.userText : MustBChatTheme.assistantText
    ...
    private var inlineAttachments: [MustBChatMessageContent] {
    ...
    private var toolCalls: [MustBChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [MustBChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? MustBChatTheme.userBubble
        }
        if self.style == .onboarding {
            return MustBChatTheme.onboardingAssistantBubble
        }
        return MustBChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return MustBChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: MustBChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? MustBChatTheme.userText : MustBChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: MustBChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(MustBChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [MustBChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: MustBChatTheme.assistantText)
            }
        }
    }
}
