...
@MainActor
public struct MustBChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: MustBChatViewModel
    ...
    private var visibleMessages: [MustBChatMessage] {
        let base: [MustBChatMessage]
    ...
    private func mergeToolResults(in messages: [MustBChatMessage]) -> [MustBChatMessage] {
    ...
    private func isToolResultMessage(_ message: MustBChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: MustBChatMessage) -> Bool {
    ...
    private func primaryText(in message: MustBChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: MustBChatMessage) -> Bool {
    ...
    private func toolCalls(in message: MustBChatMessage) -> [MustBChatMessageContent] {
    ...
    private func inlineToolResults(in message: MustBChatMessage) -> [MustBChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: MustBChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: MustBChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(MustBChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
