...
private let chatUILogger = Logger(subsystem: "ai.must-b", category: "MustBChatUI")

@MainActor
@Observable
public final class MustBChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [MustBChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [MustBChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [MustBPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [MustBChatPendingToolCall] = []
    public private(set) var sessions: [MustBChatSessionEntry] = []
    private let transport: any MustBChatTransport
    private var sessionDefaults: MustBChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: MustBChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any MustBChatTransport,
    ...
    public var sessionChoices: [MustBChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [MustBChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: MustBPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [MustBChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: MustBChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: MustBChatMessage) -> MustBChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> MustBChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return MustBChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: MustBChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [MustBChatMessage],
        incoming: [MustBChatMessage]) -> [MustBChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [MustBChatMessage]) -> [MustBChatMessage] {
        var result: [MustBChatMessage] = []
    ...
    private static func dedupeKey(for message: MustBChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [MustBChatMessageContent] = [
            MustBChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> MustBChatAttachmentPayload in
            MustBChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> MustBChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = MustBChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: MustBChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: MustBChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: MustBAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = MustBChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> MustBPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
