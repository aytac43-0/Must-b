...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> MustBChatHistoryPayload
{
    MustBChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> MustBChatSessionEntry {
    MustBChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> MustBChatSessionEntry
{
    MustBChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> MustBChatModelChoice {
    MustBChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [MustBChatHistoryPayload],
    sessionsResponses: [MustBChatSessionsListResponse] = [],
    modelResponses: [[MustBChatModelChoice]] = [],
    ...
    -> (TestChatTransport, MustBChatViewModel)
{
...
private func sendUserMessage(_ vm: MustBChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, MustBChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [MustBChatHistoryPayload]
    private let sessionsResponses: [MustBChatSessionsListResponse]
    private let modelResponses: [[MustBChatModelChoice]]
    private let resetSessionHook: (@Sendable (String) async throws -> Void)?
    private let setSessionModelHook: (@Sendable (String?) async throws -> Void)?
    private let setSessionThinkingHook: (@Sendable (String) async throws -> Void)?

    private let stream: AsyncStream<MustBChatTransportEvent>
    private let continuation: AsyncStream<MustBChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<MustBChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> MustBChatHistoryPayload {
        ...
        return self.historyResponses.last ?? MustBChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [MustBChatAttachmentPayload]) async throws -> MustBChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return MustBChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> MustBChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? MustBChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: MustBChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = MustBChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = MustBChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: MustBChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = MustBChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: MustBChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(MustBChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == MustBChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = MustBChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: MustBChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = MustBChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = MustBChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = MustBChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { MustBChatViewModel(sessionKey: "main", transport: transport) }
