...
struct ChatMessageBubble: View {
    let message: Must-bChatMessage
    let style: Must-bChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? Must-bChatTheme.userText : Must-bChatTheme.assistantText
    ...
    private var inlineAttachments: [Must-bChatMessageContent] {
    ...
    private var toolCalls: [Must-bChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [Must-bChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? Must-bChatTheme.userBubble
        }
        if self.style == .onboarding {
            return Must-bChatTheme.onboardingAssistantBubble
        }
        return Must-bChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return Must-bChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: Must-bChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? Must-bChatTheme.userText : Must-bChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: Must-bChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(Must-bChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [Must-bChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: Must-bChatTheme.assistantText)
            }
        }
    }
}
