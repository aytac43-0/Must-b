...
@MainActor
public struct Must-bChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: Must-bChatViewModel
    ...
    private var visibleMessages: [Must-bChatMessage] {
        let base: [Must-bChatMessage]
    ...
    private func mergeToolResults(in messages: [Must-bChatMessage]) -> [Must-bChatMessage] {
    ...
    private func isToolResultMessage(_ message: Must-bChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: Must-bChatMessage) -> Bool {
    ...
    private func primaryText(in message: Must-bChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: Must-bChatMessage) -> Bool {
    ...
    private func toolCalls(in message: Must-bChatMessage) -> [Must-bChatMessageContent] {
    ...
    private func inlineToolResults(in message: Must-bChatMessage) -> [Must-bChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: Must-bChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: Must-bChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(Must-bChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
