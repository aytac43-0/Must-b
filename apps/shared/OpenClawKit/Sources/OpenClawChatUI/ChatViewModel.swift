...
private let chatUILogger = Logger(subsystem: "ai.must-b", category: "Must-bChatUI")

@MainActor
@Observable
public final class Must-bChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [Must-bChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [Must-bChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [Must-bPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [Must-bChatPendingToolCall] = []
    public private(set) var sessions: [Must-bChatSessionEntry] = []
    private let transport: any Must-bChatTransport
    private var sessionDefaults: Must-bChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: Must-bChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any Must-bChatTransport,
    ...
    public var sessionChoices: [Must-bChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [Must-bChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: Must-bPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [Must-bChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: Must-bChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: Must-bChatMessage) -> Must-bChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> Must-bChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return Must-bChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: Must-bChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [Must-bChatMessage],
        incoming: [Must-bChatMessage]) -> [Must-bChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [Must-bChatMessage]) -> [Must-bChatMessage] {
        var result: [Must-bChatMessage] = []
    ...
    private static func dedupeKey(for message: Must-bChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [Must-bChatMessageContent] = [
            Must-bChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> Must-bChatAttachmentPayload in
            Must-bChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> Must-bChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = Must-bChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: Must-bChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: Must-bChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: Must-bAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = Must-bChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> Must-bPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
