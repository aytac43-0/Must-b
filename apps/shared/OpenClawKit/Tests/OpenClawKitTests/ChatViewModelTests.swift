...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> Must-bChatHistoryPayload
{
    Must-bChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> Must-bChatSessionEntry {
    Must-bChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> Must-bChatSessionEntry
{
    Must-bChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> Must-bChatModelChoice {
    Must-bChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [Must-bChatHistoryPayload],
    sessionsResponses: [Must-bChatSessionsListResponse] = [],
    modelResponses: [[Must-bChatModelChoice]] = [],
    ...
    -> (TestChatTransport, Must-bChatViewModel)
{
...
private func sendUserMessage(_ vm: Must-bChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, Must-bChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [Must-bChatHistoryPayload]
    private let sessionsResponses: [Must-bChatSessionsListResponse]
    private let modelResponses: [[Must-bChatModelChoice]]
    private let resetSessionHook: (@Sendable (String) async throws -> Void)?
    private let setSessionModelHook: (@Sendable (String?) async throws -> Void)?
    private let setSessionThinkingHook: (@Sendable (String) async throws -> Void)?

    private let stream: AsyncStream<Must-bChatTransportEvent>
    private let continuation: AsyncStream<Must-bChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<Must-bChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> Must-bChatHistoryPayload {
        ...
        return self.historyResponses.last ?? Must-bChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [Must-bChatAttachmentPayload]) async throws -> Must-bChatSendResponse
    {
        ...
        return Must-bChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> Must-bChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? Must-bChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: Must-bChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = Must-bChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = Must-bChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: Must-bChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = Must-bChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: Must-bChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(Must-bChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == Must-bChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = Must-bChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: Must-bChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = Must-bChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = Must-bChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = Must-bChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { Must-bChatViewModel(sessionKey: "main", transport: transport) }
