...
const TIER_ICONS: Record<string, React.ElementType> = {
  master: Crown, planner: Brain, worker: Wrench,
};
const TIER_DESC: Record<string, string> = {
  master: "Full autonomy", planner: "Plan & delegate", worker: "Execute tasks",
};
...
export default function RightPanel() {
  ...
  useEffect(() => {
    const handler = (ev: CustomEvent) => {
      try {
        const d = ev.detail;
        if (d?.type === "toneChange" && typeof d.tone === "string" && d.tone !== "normal") {
          setTone({
            tone:       String(d.tone),
    ...
    <aside className="w-[220px] h-screen bg-[#090c14]/80 border-l border-white/5 flex flex-col overflow-hidden sticky top-0 backdrop-blur-xl shrink-0 transition-all duration-300">
        ...
        {status ? (
          ...
          >
            ...
            <div className="min-w-0">
              <p className="leading-none text-xs">{status.role}</p>
              <p className="text-[9px] font-normal opacity-60 mt-0.5 leading-none">{TIER_DESC[tier] ?? ""}</p>
            </div>
          </motion.div>
        ) : (
        ...
        <div className="space-y-0.5">
          ...
          {status?.gpu && (
            <p className="text-[10px] text-gray-600 flex items-center gap-1 truncate" title={status.gpu}>
              <Zap size={8} /> <span className="truncate">{status.gpu}</span>
            </p>
          )}
        </div>
      </div>
      ...
      <div className="px-3 py-3 border-b border-white/5">
        ...
        <div className="flex items-start gap-1.5">
          ...
          <p className="text-[11px] font-medium text-gray-300 break-all leading-relaxed">
            {status?.model
              ? shortModel(status.model)
              : <span className="text-gray-600 italic text-[10px]">No model active</span>}
          </p>
        </div>
      </div>
            ...
            <div className="space-y-1 overflow-y-auto scrollbar-hide max-h-[130px]">
              {models.map((m, i) => {
                ...
                return (
                  ...
                  >
                    {isActive
                      ? <CheckCircle2 size={9} className="text-orange-400 shrink-0" />
                      : <span className="w-2 h-2 rounded-full bg-white/10 shrink-0" />}
                    <div className="min-w-0 flex-1">
                      <p className={`font-medium leading-none truncate text-[10px] ${isActive ? "text-orange-300" : "text-gray-400"}`}>
                        {shortModel(m.name)}
                      </p>
      ...
      <div className="px-3 pb-2 border-b border-white/5">
        <button
          onClick={async () => {
            ...
            try {
              const next = !shadowOn;
              const r = await apiFetch("/api/shadow/toggle", { method: "POST", body: JSON.stringify({ enabled: next }) });
              if (r.ok) setShadowOn(next);
            } catch { /* silent */ }
            setShadowBusy(false);
          }}
          disabled={shadowBusy}
          className={`w-full flex items-center justify-between gap-2 py-1.5 px-2.5 rounded-xl border text-[11px] font-semibold transition-all ${
            shadowOn
        ...
        >
          <div className="flex items-center gap-1.5">
            {shadowBusy ? <Loader2 size={11} className="animate-spin" /> : <Ghost size={11} className={shadowOn ? "animate-pulse" : ""} />}
            Shadow Mode
          </div>
          <div className={`w-7 h-3.5 rounded-full border transition-all relative ${shadowOn ? "bg-purple-500/40 border-purple-500/50" : "bg-white/8 border-white/10"}`}>
            <div className={`absolute top-0.5 w-2.5 h-2.5 rounded-full transition-all ${shadowOn ? "left-[14px] bg-purple-400" : "left-0.5 bg-gray-600"}`} />
          </div>
        </button>
      </div>
          ...
          {[0, 1, 2].map((slot) => (
            <button
              ...
              onClick={async () => {
                ...
                try {
                  const r = await apiFetch("/api/ghost/toggle", { method: "POST", body: JSON.stringify({ slot, enabled: next }) });
                  if (r.ok) setGhostSlots(s => { const c = [...s]; c[slot] = next; return c; });
                } catch { /* silent */ }
                setGhostBusy(b => { const c = [...b]; c[slot] = false; return c; });
              }}
              className={`flex-1 flex flex-col items-center py-1 rounded-lg border text-[9px] font-bold transition-all ${
                ghostSlots[slot]
                  ? "bg-purple-500/15 border-purple-500/30 text-purple-300"
                  : "bg-white/4 border-white/8 text-gray-600 hover:text-gray-400"
              } disabled:opacity-40`}
            >
              {ghostBusy[slot] ? <Loader2 size={8} className="animate-spin" /> : <Ghost size={8} className={ghostSlots[slot] ? "animate-pulse" : ""} />}
              G{slot + 1}
            </button>
          ))}
        </div>
      ...
      <AnimatePresence>
        {tone && (
          <motion.div
            ...
            className="px-3 pb-1"
          >
            <div className={`flex items-center gap-1.5 px-2 py-1 rounded-lg border text-[10px] font-semibold ${tone.badgeClass}`}>
      ...
      <div className="px-3 pb-2">
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 py-1.5 rounded-xl bg-orange-500/8 border border-orange-500/18 text-orange-400 text-[11px] font-semibold hover:bg-orange-500/15 hover:border-orange-500/30 transition-all"
        >
          <Smartphone size={11} />
          Connect Mobile
        </button>
