...
/** Shared local auth token — fetched once and cached */
...
async function getToken(): Promise<string | null> {
    ...
    try {
        ...
        if (r.ok) { const d = await r.json(); cachedToken = d.token; return cachedToken; }
...
export function Sidebar() {
    ...
    useEffect(() => {
        fetchChats();
        const interval = setInterval(fetchChats, 10_000); // poll every 10s
    ...
    const confirmDeleteChat = async () => {
        ...
        try {
            ...
            if (pathname.includes(deletingChatId)) router.push("/app");
                            ...
                            {editingChatId === chat.id ? (
                                <div className="flex items-center gap-1 px-2 py-1.5 bg-white/5 rounded-lg border border-orange-500/50">
                                    <input
                                        ...
                                        autoFocus
                                        onKeyDown={(e) => { if (e.key === "Enter") saveEdit(); if (e.key === "Escape") setEditingChatId(null); }}
                                    />
