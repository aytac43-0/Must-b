...
If you believe you've found a security issue in Must-b, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [must-b/must-b](https://github.com/must-b/must-b)
- **macOS desktop app** — [must-b/must-b](https://github.com/must-b/must-b) (apps/macos)
- **iOS app** — [must-b/must-b](https://github.com/must-b/must-b) (apps/ios)
- **Android app** — [must-b/must-b](https://github.com/must-b/must-b) (apps/android)
- **ClawHub** — [must-b/clawhub](https://github.com/must-b/clawhub)
- **Trust and threat model** — [must-b/trust](https://github.com/must-b/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@must-b.ai](mailto:security@must-b.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.must-b.ai).
...
- Tested version details (Must-b version and/or commit SHA).
...
- Demonstrated impact tied to Must-b's documented trust boundaries.
- For exposed-secret reports: proof the credential is Must-b-owned (or grants access to Must-b-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at Must-b. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

Must-b is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
Must-b does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Must-b can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need Must-b, use one VPS (or host/OS user boundary) per user.
...
- This is expected in Must-b's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of Must-b's trusted computing base for a gateway.
...
- Using Must-b in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.must-b`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not Must-b-owned and not granting access to Must-b-operated infrastructure/services) without demonstrated Must-b impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

Must-b security guidance assumes:

- The host where Must-b runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.must-b` state/config (including `must-b.json`) is effectively a trusted operator.
...
Must-b's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
Must-b separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when Must-b can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the Must-b process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

Must-b uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/must-b` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/must-b` (or `must-b-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the Must-b-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use Must-b temp helpers (`resolvePreferredMust-bTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-must-b-dir.ts`
...
For threat model + hardening guidance (including `must-b security audit --deep` and `--fix`), see:

- `https://docs.must-b.ai/gateway/security`
...
Must-b's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `must-b gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - Must-b keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `must-b security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__must-b__/canvas/`, `/__must-b__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__must-b__/canvas/`, `/__must-b__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

Must-b requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
docker run --read-only --cap-drop=ALL \
  -v must-b-data:/app/data \
  must-b/must-b:latest
```
