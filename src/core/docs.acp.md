...
This document describes how the Must-b ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`must-b acp` exposes an ACP agent over stdio and forwards prompts to a running
Must-b Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Must-b Gateway
...
- ACP client spawns `must-b acp` and speaks ACP messages over stdio.
...
`must-b acp` resolves the Gateway URL and auth from CLI flags or config:
