...
must-b system heartbeat last
must-b logs --follow
must-b config get agents.defaults.heartbeat
must-b channels status --probe
```
...
```bash
must-b config get agents.defaults.heartbeat.activeHours
must-b config get agents.defaults.heartbeat.activeHours.timezone
must-b config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
must-b cron list
must-b logs --follow
```
