...
Must-b routes replies **back to the channel where a message came from**. The
...
Must-b infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Must-b still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Must-b would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.must-b/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.must-b`):

- `~/.must-b/agents/<agentId>/sessions/sessions.json`
