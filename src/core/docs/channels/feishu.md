...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Must-b to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Must-b releases, so no separate plugin install
is required.
...
```bash
must-b plugins install @must-b/feishu
```
...
If you just installed Must-b, run the wizard:
...
2. Configuring app credentials in Must-b
...
1. You already ran `must-b channels add` for Feishu
2. The gateway is running (`must-b gateway status`)
...
```bash
must-b pairing approve feishu <CODE>
```
...
2. Run `must-b logs --follow` and look for `chat_id`
...
2. Run `must-b logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `must-b gateway status`  | Show gateway status           |
| `must-b gateway install` | Install/start gateway service |
| `must-b gateway stop`    | Stop gateway service          |
| `must-b gateway restart` | Restart gateway service       |
| `must-b logs --follow`   | Tail gateway logs             |
...
4. Check logs: `must-b logs --follow`
...
5. Ensure the gateway is running: `must-b gateway status`
6. Check logs: `must-b logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.must-b/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.must-b/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
