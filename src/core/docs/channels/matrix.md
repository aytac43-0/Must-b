...
Matrix is an open, decentralized messaging protocol. Must-b connects as a Matrix **user**
...
```bash
must-b plugins install @must-b/matrix
```

Local checkout (when running from a git repo):

```bash
must-b plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `must-b plugins install @must-b/matrix`
   - From a local checkout: `must-b plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Must-b calls the same
     login endpoint, stores the access token in `~/.must-b/credentials/matrix/credentials.json`,
...
- On first connection, Must-b requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Must-b logs a warning.
...
`~/.must-b/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, Must-b resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
