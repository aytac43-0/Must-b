---
summary: "Mattermost bot setup and Must-b config"
read_when:
...
```bash
must-b plugins install @must-b/mattermost
```

Local checkout (when running from a git repo):

```bash
must-b plugins install ./extensions/mattermost
```
...
4. Configure Must-b and start the gateway.
...
Native slash commands are opt-in. When enabled, Must-b registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Must-b derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Must-b.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Must-b.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Must-b, not `404`.
...
- Approve via:
  - `must-b pairing list mattermost`
  - `must-b pairing approve mattermost <CODE>`
...
Use these target formats with `must-b message send` or cron/webhooks:
...
Must-b resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Must-b sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
- If `interactions.callbackBaseUrl` is omitted, Must-b derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Must-b run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="must-b-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"must-b-mattermost-interactions",
...
`must-b message send` and cron/webhook deliveries.
