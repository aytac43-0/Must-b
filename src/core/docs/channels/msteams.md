...
```bash
must-b plugins install @must-b/msteams
```

Local checkout (when running from a git repo):

```bash
must-b plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Must-b will offer the local install path automatically.
...
3. Configure Must-b with those credentials.
...
- Talk to Must-b via Teams DMs, group chats, or channels.
...
- On startup, Must-b resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.must-b/must-b.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Must-b, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `must-b-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `must-b plugins install @must-b/msteams`
   - From a local checkout: `must-b plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Must-b in Teams", "full": "Must-b in Teams" },
...
Must-b handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Must-b only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/Must-bShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Must-b sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `must-b message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.must-b/msteams-polls.json`.
...
```bash
must-b message send --channel msteams \
...
must-b message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
must-b message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
must-b message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
must-b message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
