...
```bash
must-b plugins install @must-b/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
must-b plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Must-b" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `must-b pairing list nextcloud-talk`
  - `must-b pairing approve nextcloud-talk <CODE>`
