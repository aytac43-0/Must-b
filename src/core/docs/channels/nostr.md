...
read_when:
  - You want Must-b to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Must-b to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`must-b onboard`) and `must-b channels add` list optional channel plugins.
...
```bash
must-b plugins install @must-b/nostr
```

Use a local checkout (dev workflows):

```bash
must-b plugins install --link <path-to-must-b>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "must-b@example.com",
        "lud16": "must-b@example.com"
      }
    }
  }
}
