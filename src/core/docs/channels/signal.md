...
- Must-b installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "Must-b"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure Must-b and restart the gateway.
5. Send a first DM and approve pairing (`must-b pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "Must-b"` then scan the QR in Signal.
...
4. Configure Must-b, restart gateway, verify channel:
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `must-b pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point Must-b at it:
...
This skips auto-spawn and the startup wait inside Must-b. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `must-b pairing list signal`
  - `must-b pairing approve signal <CODE>`
...
- **Typing indicators**: Must-b sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, Must-b forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `must-b doctor --fix`.
...
must-b pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/must-b/must-b-$(date +%Y-%m-%d).log" | tail -20
```
