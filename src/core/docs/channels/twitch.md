...
read_when:
  - Setting up Twitch chat integration for Must-b
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Must-b connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
must-b plugins install @must-b/twitch
```

Local checkout (when running from a git repo):

```bash
must-b plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `MUSTB_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "must-b", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use MUSTB_TWITCH_ACCESS_TOKEN env var)
...
```bash
MUSTB_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
