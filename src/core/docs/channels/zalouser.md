...
read_when:
  - Setting up Zalo Personal for Must-b
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Must-b.
...
- Install via CLI: `must-b plugins install @must-b/zalouser`
- Or from a source checkout: `must-b plugins install ./extensions/zalouser`
...
- `must-b pairing list zalouser`
- `must-b pairing approve zalouser <code>`
...
- On startup, Must-b resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Must-b stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Must-b state. Example:
...
- Must-b sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Must-b sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `must-b channels logout --channel zalouser && must-b channels login --channel zalouser`
...
- The channel now runs fully in Must-b without external CLI binaries.
