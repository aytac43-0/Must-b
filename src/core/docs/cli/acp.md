...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a Must-b Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`must-b acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Must-b gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
must-b acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
must-b acp --url wss://gateway-host:18789 --token-file ~/.must-b/gateway.token

# Attach to an existing session key
must-b acp --session agent:main:main

# Attach by label (must already exist)
must-b acp --session-label "support inbox"

# Reset the session key before the first prompt
must-b acp --session agent:main:main --reset-session
```
...
must-b acp client --server-args --url wss://gateway-host:18789 --token-file ~/.must-b/gateway.token

# Override the server command (default: must-b)
must-b acp client --server "node" --server-args must-b.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive a Must-b Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `must-b acp` over stdio.

Example config (persisted):

```bash
must-b config set gateway.remote.url wss://gateway-host:18789
must-b config set gateway.remote.token <token>
```
...
```bash
must-b acp --session agent:main:main
must-b acp --session agent:design:main
must-b acp --session agent:qa:bug-123
```
...
Must-b bot over ACP, use `acpx` with its built-in `must-b` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx must-b` at `must-b acp`.
3. Target the Must-b session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Must-b ACP session
acpx must-b exec "Summarize the active Must-b session state."

# Persistent named session for follow-up turns
acpx must-b sessions ensure --name codex-bridge
acpx must-b -s codex-bridge --cwd /path/to/repo \
  "Ask my Must-b work agent for recent context relevant to this repo."
```

If you want `acpx must-b` to target a specific Gateway and session key every
time, override the `must-b` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "must-b": {
      "command": "env MUSTB_HIDE_BANNER=1 MUSTB_SUPPRESS_NOTES=1 must-b acp --url ws://127.0.0.1:18789 --token-file ~/.must-b/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Must-b checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env MUSTB_HIDE_BANNER=1 MUSTB_SUPPRESS_NOTES=1 node must-b.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Must-b agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Must-b ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`MUSTB_GATEWAY_TOKEN`, `MUSTB_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`MUSTB_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `MUSTB_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `must-b acp client` sets `MUSTB_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `must-b`).
