---
summary: "CLI reference for `must-b configure` (interactive configuration prompts)"
read_when:
...
Tip: `must-b config` without a subcommand opens the same wizard. Use
`must-b config get|set|unset` for non-interactive edits.
...
```bash
must-b configure
must-b configure --section model --section channels
```
