---
summary: "CLI reference for `must-b onboard` (interactive onboarding wizard)"
read_when:
...
```bash
must-b onboard
must-b onboard --flow quickstart
must-b onboard --flow manual
must-b onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`MUSTB_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `must-b gateway run`.
...
- Fastest first chat: `must-b dashboard` (Control UI, no channel setup).
