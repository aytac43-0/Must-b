...
- `/usage cost` in chats: local cost summary aggregated from Must-b session logs.
- CLI: `must-b status --usage` prints a full per-provider breakdown.
- CLI: `must-b channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
