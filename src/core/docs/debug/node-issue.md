...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Must-b
...
Running Must-b via Node with `tsx` fails at startup with:

```
[must-b] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `must-b status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch must-b.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node must-b.mjs status` works on Node 25.
