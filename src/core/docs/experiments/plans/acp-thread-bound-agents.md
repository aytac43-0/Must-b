...
This plan defines how Must-b should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Must-b core owns ACP control plane concerns
...
Must-b should not reimplement ACP transport internals in core.
Must-b should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Must-b, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Must-b state dir
...
- ACP event stream is normalized to Must-b reply chunks
...
For the first production backend (`extensions/acpx`), Must-b and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Must-b `sessionKey`)
...
- Must-b consumes ndjson events from `acpx --format json --json-strict`
