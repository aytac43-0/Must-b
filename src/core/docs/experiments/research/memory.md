...
read_when:
  - Designing workspace memory (~/.must-b/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Must-b integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.must-b/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.must-b/workspace` as canonical human-readable memory.
...
```
~/.must-b/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Must-b**, but keep a separable core library.

### Why integrate into Must-b?

- Must-b already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `must-b memory recall "…" --k 25 --since 30d`
  - `must-b memory reflect --since 7d`
...
Pragmatic take for `~/.must-b/workspace`:
