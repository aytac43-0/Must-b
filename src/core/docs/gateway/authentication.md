...
Must-b supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `must-b gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.must-b/.env` so the daemon can read it:
...
API keys for daemon use: `must-b onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.must-b/.env`, systemd/launchd).
...
```bash
must-b models auth paste-token --provider anthropic
must-b models auth paste-token --provider openrouter
```
...
- Priority order:
  - `MUSTB_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Must-b retries with the next key only for rate-limit errors (for example
...
Run `must-b models status` to confirm which profile is expiring. If the profile
