...
description: "Complete field-by-field reference for ~/.must-b/must-b.json"
summary: "Complete reference for every Must-b config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.must-b/must-b.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Must-b uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.must-b/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `must-b doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `must-b doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["must-b-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-must-b",
          requireMention: false,
...
- Must-b additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `must-b plugins install @must-b/mattermost`.
...
- `commands.callbackUrl` must resolve to the Must-b gateway endpoint and be reachable from the Mattermost server.
...
Must-b spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `must-b channels add` (or channel onboarding) while still on a single-account top-level channel config, Must-b moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `must-b doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@must-b", "must-b"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@must-b"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `must-b.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Must-b auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/must-b" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Must-b assumes `anthropic` (deprecated).
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.must-b/sandboxes",
        docker: {
          image: "must-b-sandbox:bookworm-slim",
          containerPrefix: "must-b-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "must-b-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "must-b-sandbox-browser:bookworm-slim",
          network: "must-b-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.must-b/sandboxes`
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `must-b.json`.
noVNC observer access uses VNC auth by default and Must-b emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `must-b-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from MUSTB_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `MUSTB_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `MUSTB_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `MUSTB_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.must-b/workspace",
        agentDir: "~/.must-b/agents/main/agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.must-b/workspace-home" },
      { id: "work", workspace: "~/.must-b/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, Must-b resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.must-b/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.must-b/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.must-b/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.must-b/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Must-b starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.must-b/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Must-b treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:must-b`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.must-b/attachments/<uuid>/` with a `.manifest.json`.
...
Must-b uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.must-b/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `MUSTB_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Must-b forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `must-b onboard --auth-choice opencode-zen` or `must-b onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `must-b onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `must-b onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `must-b onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `must-b onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `must-b onboard --auth-choice minimax-api`.
...
- Loaded from `~/.must-b/extensions`, `<workspace>/.must-b/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `must-b plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or MUSTB_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `MUSTB_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `MUSTB_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `MUSTB_APNS_RELAY_BASE_URL` / `MUSTB_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `MUSTB_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
Convenience flags: `--dev` (uses `~/.must-b-dev` + port `19001`), `--profile <name>` (uses `~/.must-b-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.must-b/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-must-b-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `MUSTB_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.must-b/workspace/canvas",
    liveReload: true,
    // enabled: false, // or MUSTB_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__must-b__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__must-b__/a2ui/`
...
- Also serves A2UI at `/__must-b__/a2ui/`.
...
- Hostname defaults to `must-b`. Override with `MUSTB_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.must-b/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `must-b dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.must-b/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${MUSTB_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `must-b.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.must-b/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/must-b-vault-resolver",
...
- Legacy OAuth imports from `~/.must-b/credentials/oauth.json`.
...
- Default log file: `/tmp/must-b/must-b-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Must-b.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `MUSTB_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `must-b doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `must-b.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
