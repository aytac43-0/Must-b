...
read_when:
  - Setting up Must-b for the first time
...
Must-b reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.must-b/must-b.json`.

If the file is missing, Must-b uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `must-b onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    must-b onboard       # full setup wizard
    must-b configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.must-b/must-b.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Must-b only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`must-b doctor`, `must-b logs`, `must-b health`, `must-b status`)
- Run `must-b doctor` to see exact issues
- Run `must-b doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `must-b.json`.
    ...
    ```bash
    must-b config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `MUSTB_APNS_RELAY_BASE_URL` and `MUSTB_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `MUSTB_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.must-b/must-b.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `must-b config set` for single keys.
    </Warning>
    ...
    must-b gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.must-b/workspace\" } } }",
      "baseHash": "<hash>",
...
Must-b reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.must-b/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Must-b runs your login shell and imports only the missing keys:
...
Env var equivalent: `MUSTB_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${MUSTB_GATEWAY_TOKEN}" } },
