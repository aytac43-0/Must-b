...
`must-b doctor` is the repair + migration tool for Must-b. It fixes stale
...
```bash
must-b doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
must-b doctor --repair --force
```
...
- Extra workspace dir detection (`~/must-b`).
...
you to run `must-b doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.must-b/must-b.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.must-b/sessions/` to `~/.must-b/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.must-b/agent/` to `~/.must-b/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.must-b/credentials/*.json` (except `oauth.json`)
  - to `~/.must-b/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `must-b doctor`.

### 3b) Legacy cron store migrations

Doctor also checks the cron job store (`~/.must-b/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.must-b` folders exist across
  home directories or when `MUSTB_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.must-b/must-b.json` is
...
offers to remove them and install the Must-b service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Must-b gateway services are considered first-class and are not
flagged as "extra."
...
- `must-b doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `must-b doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
- `must-b doctor` prompts before rewriting supervisor config.
- `must-b doctor --yes` accepts the default repair prompts.
- `must-b doctor --repair` applies recommended fixes without prompts.
- `must-b doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `must-b gateway install --force`.
