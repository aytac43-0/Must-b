...
- During heartbeat runs, Must-b treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `must-b sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Must-b skips the heartbeat run entirely (no model call).
...
```bash
must-b system event --text "Check for urgent follow-ups" --mode now
```
