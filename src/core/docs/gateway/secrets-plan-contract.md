...
read_when:
  - Generating or reviewing `must-b secrets apply` plans
...
This page defines the strict contract enforced by `must-b secrets apply`.

If a target does not match these rules, apply fails before mutating configuration.

## Plan file shape

`must-b secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
- `secrets apply` writes supported `must-b.json` targets, supported `auth-profiles.json` targets, and optional scrub targets.
...
If apply fails with an invalid target path message, regenerate the plan with `must-b secrets configure` or fix the target path to a supported shape above.
