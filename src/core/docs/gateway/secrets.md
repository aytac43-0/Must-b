...
Must-b supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `MUSTB_GATEWAY_TOKEN` (or `CLAWDBOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Must-b runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Must-b validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Must-b QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/must-b"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `must-b.json` refs)
...
When reload-time activation fails after a healthy state, Must-b enters degraded secrets state.
...
- Strict command paths (for example `must-b memory` remote-memory paths and `must-b qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `must-b status`, `must-b status --all`, `must-b channels status`, `must-b channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `must-b secrets reload`.
...
```bash
must-b secrets audit --check
must-b secrets configure
must-b secrets audit --check
```
...
- plaintext values at rest (`must-b.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `must-b.json` refs)
...
- lets you select supported secret-bearing fields in `must-b.json` plus `auth-profiles.json` for one agent scope
...
- `must-b secrets configure --providers-only`
- `must-b secrets configure --skip-provider-setup`
- `must-b secrets configure --agent <id>`
...
```bash
must-b secrets apply --from /tmp/must-b-secrets-plan.json
must-b secrets apply --from /tmp/must-b-secrets-plan.json --dry-run
```
...
Must-b intentionally does not write rollback backups containing historical plaintext secret values.
