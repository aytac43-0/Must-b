...
- `must-b gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `must-b doctor` reports no blocking config/service issues.
- `must-b channels status --probe` shows connected/ready channels.
...
must-b config get agents.defaults.models
```
...
must-b status
must-b channels status --probe
must-b pairing list --channel <channel> [--account <id>]
must-b config get channels
must-b logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `must-b config get gateway.auth.token` then paste into Control UI settings.                          |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `must-b devices list` then `must-b devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `must-b configure`). If you are running Must-b via Podman using the dedicated `must-b` user, the config lives at `~must-b/.must-b/must-b.json`.
...
must-b cron status
must-b cron list
must-b cron runs --id <jobId> --limit 20
must-b system heartbeat last
must-b logs --follow
```
...
must-b approvals get --node <idOrNameOrIp>
must-b logs --follow
must-b status
```
...
must-b browser start --browser-profile must-b
...
```bash
must-b gateway status
must-b config get gateway.mode
must-b config get gateway.remote.url
must-b config get gateway.auth.mode
```
...
```bash
must-b config get gateway.bind
