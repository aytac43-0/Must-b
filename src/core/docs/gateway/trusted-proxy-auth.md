...
read_when:
  - Running Must-b behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Must-b
...
- You run Must-b behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Must-b checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Must-b extracts the user identity from the configured header
...
- Must-b can stay on loopback HTTP behind the proxy.
...
If Must-b itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://must-b.example.com
    to: http://must-b-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`must-b security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Must-b config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `must-b security audit` and review findings
