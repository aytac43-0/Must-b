---
summary: "Deploy Must-b Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Must-b on a Kubernetes cluster
  - You want to test Must-b in a Kubernetes environment
title: "Kubernetes"
---

# Must-b on Kubernetes

A minimal starting point for running Must-b on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Must-b is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/must-b 18789:18789 -n must-b
...
```bash
kubectl get secret must-b-secrets -n must-b -o jsonpath='{.data.MUSTB_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: must-b (configurable via MUSTB_NAMESPACE)
├── Deployment/must-b        # Single pod, init container + gateway
├── Service/must-b           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/must-b-config  # must-b.json + AGENTS.md
└── Secret/must-b-secrets    # Gateway token + API keys
```
...
Edit `must-b.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret must-b-secrets -n must-b \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/must-b -n must-b
```

### Custom namespace

```bash
MUSTB_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/must-b/must-b:2026.3.1
```
...
└── manifests/
    ...
    ├── configmap.yaml          # must-b.json + AGENTS.md
