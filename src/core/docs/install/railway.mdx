...
Deploy Must-b on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/must-b` — Control UI
...
- `MUSTB_STATE_DIR=/data/.must-b` (recommended)
- `MUSTB_WORKSPACE_DIR=/data/workspace` (recommended)
- `MUSTB_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Must-b state + workspace so you can migrate to another host without losing config or memory.
