...
Deploy Must-b on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/must-b/must-b)
...
repository configures everything needed to run Must-b:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: MUSTB_STATE_DIR
        value: /data/.must-b
      - key: MUSTB_WORKSPACE_DIR
        value: /data/workspace
      - key: MUSTB_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/must-b`.
...
If you use the original Must-b repository, Render will not auto-deploy your Must-b. To update it, run a manual Blueprint sync from the dashboard.
...
For Must-b, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Must-b host.
