...
Must-b supports **camera capture** for agent workflows:
...
```bash
must-b nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
must-b nodes camera snap --node <id> --facing front
must-b nodes camera clip --node <id> --duration 3000
must-b nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`must-b.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `must-b` CLI to invoke camera commands on the macOS node.

Examples:

```bash
must-b nodes camera list --node <id>            # list camera ids
must-b nodes camera snap --node <id>            # prints MEDIA:<path>
must-b nodes camera snap --node <id> --max-width 1280
must-b nodes camera snap --node <id> --delay-ms 2000
must-b nodes camera snap --node <id> --device-id <id>
must-b nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
must-b nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
must-b nodes camera clip --node <id> --device-id <id>
must-b nodes camera clip --node <id> --no-audio
```

Notes:

- `must-b nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
must-b nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
