...
Must-b supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Must-b also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `must-b configure --section web`. It stores the key in
`~/.must-b/must-b.json` under `tools.web.search.perplexity.apiKey`.
...
`~/.must-b/.env` (or your service environment). See [Env vars](/help/faq#how-does-must-b-load-environment-variables).
