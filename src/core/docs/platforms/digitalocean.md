---
summary: "Must-b on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Must-b on DigitalOcean
  - Looking for cheap VPS hosting for Must-b
title: "DigitalOcean"
---

# Must-b on DigitalOcean

## Goal

Run a persistent Must-b Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status must-b-gateway.service

# View logs
journalctl --user -u must-b-gateway.service -f
```
...
```bash
must-b config set gateway.bind tailnet
must-b gateway restart
```
...
```bash
must-b channels login whatsapp
# Scan QR code
```
...
- `~/.must-b/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u must-b --no-pager -n 50
```
