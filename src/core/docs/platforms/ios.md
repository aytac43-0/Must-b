...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `must-b.internal.`), **or**
...
```bash
must-b nodes status
must-b gateway call node.list --params "{}"
```
...
- `MUSTB_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Must-b iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Must-b builds.
...
```bash
export MUSTB_APNS_TEAM_ID="TEAMID"
export MUSTB_APNS_KEY_ID="KEYID"
export MUSTB_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
The Gateway advertises `_must-b-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `must-b.internal.`) and Tailscale split DNS.
...
```bash
must-b nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__must-b__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__must-b__/canvas/` and `/__must-b__/a2ui/`.
...
```bash
must-b nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__must-b; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
must-b nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `must-b devices list` and approve manually.
