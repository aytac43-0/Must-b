...
service via the external `must-b` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.must-b.gateway`
  (or `ai.must-b.<profile>` when using `--profile`/`MUSTB_PROFILE`; legacy `com.must-b.*` is supported).
