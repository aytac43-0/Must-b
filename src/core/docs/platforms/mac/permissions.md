...
- Same path: run the app from a fixed location (for Must-b, `dist/Must-b.app`).
...
```bash
sudo tccutil reset Accessibility ai.must-b.mac
sudo tccutil reset ScreenCapture ai.must-b.mac
...
Workaround: move files into the Must-b workspace (`~/.must-b/workspace`) if you want to avoid per-folder grants.
