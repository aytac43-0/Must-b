---
summary: "Must-b on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Must-b on a Raspberry Pi
  - Running Must-b on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Must-b on Raspberry Pi

## Goal

Run a persistent, always-on Must-b Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u must-b -f
```

## 9) Access the Must-b Dashboard
...
```bash
ssh user@gateway-host 'must-b dashboard --no-open'
```
...
(or `MUSTB_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/must-b-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/must-b-compile-cache
mkdir -p /var/tmp/must-b-compile-cache
export MUSTB_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `MUSTB_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Must-b, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit must-b
```

```ini
[Service]
Environment=MUSTB_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/must-b-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Must-b state/cache on SSD-backed storage to avoid SD-card
...
Most Must-b features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled must-b

# Enable if not
sudo systemctl enable must-b

# Start on boot
sudo systemctl start must-b
```
...
```bash
# Check logs
journalctl -u must-b --no-pager -n 100

# Common fix: rebuild
cd ~/must-b  # if using hackable install
npm run build
sudo systemctl restart must-b
```
