---
summary: "Use MiniMax M2.5 in Must-b"
read_when:
  - You want MiniMax models in Must-b
...
```bash
must-b plugins enable minimax-portal-auth  # skip if already loaded.
must-b gateway restart  # restart if gateway is already running
must-b onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/must-b/must-b/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `must-b.json`:
...
- Use `must-b models list` and `must-b models set minimax/MiniMax-M2.5` to switch.
...
- Running `must-b configure` and selecting **MiniMax M2.5**, or
