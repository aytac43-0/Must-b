---
summary: "Use OpenAI via API keys or Codex subscription in Must-b"
read_when:
  - You want to use OpenAI models in Must-b
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Must-b.
...
```bash
must-b onboard --auth-choice openai-api-key
# or non-interactive
must-b onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. Must-b forwards both through the `openai/*` Responses path.
Must-b intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

Must-b does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in Must-b.
...
must-b onboard --auth-choice openai-codex

# Or run OAuth directly
must-b models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Must-b
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

If your Codex account is entitled to Codex Spark, Must-b also supports:

- `openai-codex/gpt-5.3-codex-spark`

Must-b treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

Must-b also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
Must-b uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Must-b also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Must-b enables it by default for
...
Must-b, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
Must-b exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, Must-b applies a low-latency OpenAI profile:
...
`baseUrl` on `api.openai.com`), Must-b now auto-enables OpenAI server-side
