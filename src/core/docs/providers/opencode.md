---
summary: "Use OpenCode Zen and Go catalogs with Must-b"
read_when:
...
OpenCode exposes two hosted catalogs in Must-b:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Must-b keeps the runtime provider ids
...
must-b onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
must-b onboard --auth-choice opencode-go
must-b onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
