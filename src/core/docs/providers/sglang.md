---
summary: "Run Must-b with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Must-b against a local SGLang server
...
Must-b can connect to SGLang using the `openai-completions` API.

Must-b can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Must-b will query:
