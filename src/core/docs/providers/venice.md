---
summary: "Use Venice AI privacy-focused models in Must-b"
read_when:
  - You want privacy-focused inference in Must-b
...
```bash
must-b onboard --auth-choice venice-api-key
```
...
```bash
must-b agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Must-b shows all available Venice models. Pick based on your needs:
...
```bash
must-b models set venice/kimi-k2-5
must-b models set venice/claude-opus-4-6
```

List all available models:

```bash
must-b models list | grep venice
```

## Configure via `must-b configure`

1. Run `must-b configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Must-b automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
must-b agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
must-b agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
must-b agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
must-b agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
must-b agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `must-b models list` to see currently available models. Some models may be temporarily offline.
