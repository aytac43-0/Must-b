...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.must-b/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.must-b/node.json` (node id + pairing token).
...
Path: `~/.must-b/exec-approvals.json`
...
- Unix socket at `~/.must-b/exec-approvals.sock` (0600).
