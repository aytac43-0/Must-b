...
- Publish as `must-b/plugin-sdk` (or export from core under `must-b/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `Must-bPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: Must-bConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: Must-bConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: Must-bConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: Must-bConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: Must-bConfig): string;
  };
};
```
...
- Introduce `must-b/plugin-sdk`.
- Add `api.runtime` to `Must-bPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `must-bRuntime: ">=2026.2.0"`).
