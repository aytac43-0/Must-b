...
Must-b features that can generate provider usage or paid API calls.
...
- `must-b status --usage` and `must-b channels list` show provider **usage windows**
...
Must-b can pick up credentials from:
...
`must-b models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
