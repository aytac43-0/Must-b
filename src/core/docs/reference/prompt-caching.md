...
- With Anthropic API-key auth profiles, Must-b seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Must-b injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Must-b exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.must-b/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$MUSTB_STATE_DIR/logs/cache-trace.jsonl`
...
- `MUSTB_CACHE_TRACE=1` enables cache tracing.
- `MUSTB_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `MUSTB_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `MUSTB_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `MUSTB_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
