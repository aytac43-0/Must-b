...
Must-b integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Must-b owns lifecycle when `channels.signal.autoStart=true`.
...
- Must-b spawns `imsg rpc` as a child process (legacy iMessage integration).
