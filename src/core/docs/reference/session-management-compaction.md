...
This document explains how Must-b manages sessions end-to-end:
...
Must-b is designed around a single **Gateway process** that owns session state.
...
Must-b persists sessions in two layers:
...
- Transcripts: `~/.must-b/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Must-b resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Must-b reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.must-b/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Must-b intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Must-b consumes the events, but Pi decides when to compact).
...
Must-b also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Must-b bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Must-b leaves it alone.
...
Must-b supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Must-b strips/suppresses this in the delivery layer.

As of `2026.1.10`, Must-b also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Must-b uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Must-b’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `must-b status`.
