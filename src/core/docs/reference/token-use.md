---
summary: "How Must-b builds prompt context and reports token usage + costs"
read_when:
...
Must-b tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Must-b assembles its own system prompt on every run. It includes:
...
For images, Must-b downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Must-b session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `must-b status --usage` and `must-b channels list` show
...
`cacheWrite`. If pricing is missing, Must-b shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Must-b can
...
Anthropic's 1M context window is currently beta-gated. Must-b can inject the
...
Must-b skips the `context-1m-*` beta header because Anthropic currently
