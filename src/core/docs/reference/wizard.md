...
This is the full reference for the `must-b onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.must-b/credentials/oauth.json`; auth profiles live in `~/.must-b/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.must-b/credentials/oauth.json` (or `$MUSTB_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.must-b/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `must-b pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `must-b configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `must-b health`.
    - Tip: `must-b status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export MUSTB_GATEWAY_TOKEN="your-token"
must-b onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env MUSTB_GATEWAY_TOKEN
```
...
- Stores it under `~/.must-b/tools/signal-cli/<version>/`.
...
Typical fields in `~/.must-b/must-b.json`:
...
`must-b agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.must-b/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.must-b/agents/<agentId>/sessions/`.
