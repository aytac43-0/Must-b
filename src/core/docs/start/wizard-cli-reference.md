...
read_when:
  - You need detailed behavior for must-b onboard
...
This page is the full reference for `must-b onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.must-b/must-b.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `must-b doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `must-b pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `must-b health`.
    - `must-b status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.must-b/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.must-b/agents/<agentId>/agent/auth-profiles.json`
...
`~/.must-b/credentials/oauth.json` (or `$MUSTB_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`must-b onboard`](/cli/onboard)
