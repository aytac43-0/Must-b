...
The onboarding wizard is the **recommended** way to set up Must-b on macOS,
...
`must-b dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`must-b configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.must-b/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `must-b doctor` first.
</Note>
...
Use `must-b agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.must-b/workspace-<agentId>`.
...
- CLI command reference: [`must-b onboard`](/cli/onboard)
