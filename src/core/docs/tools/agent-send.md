---
summary: "Direct `must-b agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `must-b agent` (direct agent runs)

`must-b agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `must-b message --target`).
...
```bash
must-b agent --to +15555550123 --message "status update"
...
must-b agent --to +15555550123 --message "Summon reply" --deliver
