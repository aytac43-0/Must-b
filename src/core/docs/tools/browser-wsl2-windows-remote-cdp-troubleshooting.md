...
read_when:
  - Running Must-b Gateway in WSL2 while Chrome lives on Windows
...
- Must-b Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/must-b/must-b/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
Use the built-in `chrome` profile plus the Must-b Chrome extension.
...
- Must-b points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Must-b is not the problem yet.
...
Fix that before touching Must-b config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Must-b at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Must-b to succeed
...
```bash
must-b browser open https://example.com --browser-profile remote
must-b browser tabs --browser-profile remote
```

For the extension relay:

```bash
must-b browser tabs --browser-profile chrome
```

Good result:

- the tab opens in Windows Chrome
- `must-b browser tabs` returns the target
...
3. Must-b config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Must-b config or Control UI auth
