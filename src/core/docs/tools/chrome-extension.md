---
summary: "Chrome extension: let Must-b drive your existing Chrome tab"
read_when:
...
The Must-b Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate must-b-managed Chrome profile.
...
Must-b then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Must-b release (npm package) as static files. There is no separate “build” step.

After upgrading Must-b:

- Re-run `must-b browser extension install` to refresh the installed files under your Must-b state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Must-b ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `MUSTB_GATEWAY_TOKEN`)
...
- Open the tab you want Must-b to control.
...
- `ON`: attached; Must-b can drive that tab.
...
Debugging: `must-b sandbox explain`
...
`must-b browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `must-b browser extension install` first to copy the extension to a stable location under your Must-b state directory.
...
- **This is not isolated** like the dedicated must-b-managed profile.
