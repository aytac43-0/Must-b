...
- **gateway host** → `must-b` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Must-b also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.must-b/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.must-b/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `must-b approvals allowlist ...`).
- `must-b security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `must-b doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.must-b/exec-approvals.json` directly.

CLI: `must-b approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, Must-b does not treat
...
topics, Must-b preserves the topic for the approval prompt and the post-approval follow-up.
