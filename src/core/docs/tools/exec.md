...
- `gateway`/`node` approvals are controlled by `~/.must-b/exec-approvals.json`.
...
- Must-b sets `MUSTB_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Must-b prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`must-b security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `must-b doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
