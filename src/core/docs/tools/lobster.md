---
title: Lobster
summary: "Typed workflow runtime for Must-b with resumable approval gates."
description: Typed workflow runtime for Must-b — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets Must-b run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Must-b runs one Lobster tool call and gets a structured result.
...
Must-b launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | must-b.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
must-b.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Must-b tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Must-b Gateway (see the [Lobster repo](https://github.com/must-b/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Must-b keeps core tools enabled. To restrict core
...
→ must-b calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ must-b calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Must-b tools that do.
