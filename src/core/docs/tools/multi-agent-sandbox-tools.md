...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `must-b sandbox explain`.
...
Legacy `agent.*` configs are migrated by `must-b doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=must-b-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${MUSTB_STATE_DIR:-$HOME/.must-b}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
