---
summary: "Must-b plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends Must-b with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core Must-b yet (or you want to keep optional features out of your main
install).
...
```bash
must-b plugins install @must-b/voice-call
```
...
Must-b's plugin system has four layers:

1. **Manifest + discovery**
   Must-b finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads
   `must-b.plugin.json` plus package metadata first.
...
4. **Surface consumption**
   The rest of Must-b reads the registry to expose tools, channels, provider
...
That split lets Must-b validate config, explain missing/disabled plugins, and
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@must-b/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@must-b/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@must-b/zalouser`
- [Matrix](/channels/matrix) — `@must-b/matrix`
- [Nostr](/channels/nostr) — `@must-b/nostr`
- [Zalo](/channels/zalo) — `@must-b/zalo`
- [Microsoft Teams](/channels/msteams) — `@must-b/msteams`
- Google Antigravity OAuth (provider auth) — bundled as `google-antigravity-auth` (disabled by default)
- Gemini CLI OAuth (provider auth) — bundled as `google-gemini-cli-auth` (disabled by default)
- Qwen OAuth (provider auth) — bundled as `qwen-portal-auth` (disabled by default)
- Copilot Proxy (provider auth) — local VS Code Copilot Proxy bridge; distinct from built-in `github-copilot` device login (bundled, disabled by default)

Must-b plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
At startup, Must-b does roughly this:

1. discover candidate plugin roots
2. read `must-b.plugin.json` and package metadata
...
The manifest is the control-plane source of truth. Must-b uses it to:
...
Must-b keeps short in-process caches for:
...
Use SDK subpaths instead of the monolithic `must-b/plugin-sdk` import when
authoring plugins:

- `must-b/plugin-sdk/core` for generic plugin APIs, provider auth types, and shared helpers.
- `must-b/plugin-sdk/compat` for bundled/internal plugin code that needs broader shared runtime helpers than `core`.
- `must-b/plugin-sdk/telegram` for Telegram channel plugins.
- `must-b/plugin-sdk/discord` for Discord channel plugins.
- `must-b/plugin-sdk/slack` for Slack channel plugins.
- `must-b/plugin-sdk/signal` for Signal channel plugins.
- `must-b/plugin-sdk/imessage` for iMessage channel plugins.
- `must-b/plugin-sdk/whatsapp` for WhatsApp channel plugins.
- `must-b/plugin-sdk/line` for LINE channel plugins.
- `must-b/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Bundled extension-specific subpaths are also available:
  `must-b/plugin-sdk/acpx`, `must-b/plugin-sdk/bluebubbles`,
  `must-b/plugin-sdk/copilot-proxy`, `must-b/plugin-sdk/device-pair`,
  `must-b/plugin-sdk/diagnostics-otel`, `must-b/plugin-sdk/diffs`,
  `must-b/plugin-sdk/feishu`,
  `must-b/plugin-sdk/google-gemini-cli-auth`, `must-b/plugin-sdk/googlechat`,
  `must-b/plugin-sdk/irc`, `must-b/plugin-sdk/llm-task`,
  `must-b/plugin-sdk/lobster`, `must-b/plugin-sdk/matrix`,
  `must-b/plugin-sdk/mattermost`, `must-b/plugin-sdk/memory-core`,
  `must-b/plugin-sdk/memory-lancedb`,
  `must-b/plugin-sdk/minimax-portal-auth`,
  `must-b/plugin-sdk/nextcloud-talk`, `must-b/plugin-sdk/nostr`,
  `must-b/plugin-sdk/open-prose`, `must-b/plugin-sdk/phone-control`,
  `must-b/plugin-sdk/qwen-portal-auth`, `must-b/plugin-sdk/synology-chat`,
  `must-b/plugin-sdk/talk-voice`, `must-b/plugin-sdk/test-utils`,
  `must-b/plugin-sdk/thread-ownership`, `must-b/plugin-sdk/tlon`,
  `must-b/plugin-sdk/twitch`, `must-b/plugin-sdk/voice-call`,
  `must-b/plugin-sdk/zalo`, and `must-b/plugin-sdk/zalouser`.

Compatibility note:

- `must-b/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `must-b status`, `must-b status --all`,
  `must-b channels status`, `must-b channels resolve`, and doctor/config
...
- Set `MUSTB_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `MUSTB_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `MUSTB_PLUGIN_DISCOVERY_CACHE_MS` and
  `MUSTB_PLUGIN_MANIFEST_CACHE_MS`.

## Discovery & precedence

Must-b scans, in order:
...
- `<workspace>/.must-b/extensions/*.ts`
- `<workspace>/.must-b/extensions/*/index.ts`

3. Global extensions

- `~/.must-b/extensions/*.ts`
- `~/.must-b/extensions/*/index.ts`

4. Bundled extensions (shipped with Must-b, mostly disabled by default)

- `<must-b>/extensions/*`

Most bundled plugins must be enabled explicitly via
`plugins.entries.<id>.enabled` or `must-b plugins enable <id>`.
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, Must-b logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. Must-b blocks candidates when:
...
Each plugin must include a `must-b.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `must-b.extensions`:
...
Security guardrail: every `must-b.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `must-b plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `must-b.channel` and
install hints via `must-b.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@must-b/nextcloud-talk",
  "must-b": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
...
Must-b can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.must-b/mpm/plugins.json`
- `~/.must-b/mpm/catalog.json`
- `~/.must-b/plugins/catalog.json`

Or point `MUSTB_PLUGIN_CATALOG_PATHS` (or `MUSTB_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "must-b": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, Must-b uses it but warns when it doesn’t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `must-b.plugin.json` (`configSchema`).
...
Must-b preserves config for disabled plugins so toggling them back on is not
destructive.
...
Must-b augments `uiHints` at runtime based on discovered plugins:
...
must-b plugins install <path>                 # copy a local file/dir into ~/.must-b/extensions/<id>
must-b plugins install ./extensions/voice-call # relative path ok
must-b plugins install ./plugin.tgz           # install from a local tarball
must-b plugins install ./plugin.zip           # install from a local zip
must-b plugins install -l ./extensions/voice-call # link (no copy) for dev
must-b plugins install @must-b/voice-call # install from npm
must-b plugins install @must-b/voice-call --pin # store exact resolved name@version
...
If stored integrity metadata changes between updates, Must-b warns and asks for confirmation (use global `--yes` to bypass prompts).

Plugins may also register their own top‑level commands (example: `must-b voicecall`).
...
- Plugin-managed hooks show up in `must-b hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `must-b hooks`; enable/disable the plugin instead.
...
- When disabled, Must-b blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
setup inside Must-b, surface provider setup in onboarding/model-pickers, and
...
2. **Non-interactive setup**
   `auth[].runNonInteractive(ctx)` handles `must-b onboard --non-interactive`
   without prompts. Use this when the provider needs custom headless setup
   beyond the built-in simple API-key paths.
3. **Wizard integration**
   `wizard.onboarding` adds an entry to `must-b onboard`.
...
- `configPatch`: optional `must-b.json` changes
...
explicit method or let Must-b synthesize per-method choices.

Must-b validates provider wizard metadata when the plugin registers:
...
- `must-b models auth login --provider <id> [--method <id>]`
- `must-b onboard`
...
- `wizard.onboarding` adds a provider choice to `must-b onboard`.
...
- `config`: The current Must-b config
...
- Main package: `must-b` (this repo)
- Plugins: separate npm packages under `@must-b/*` (example: `@must-b/voice-call`)

Publishing contract:

- Plugin `package.json` must include `must-b.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `must-b plugins install <npm-spec>` uses `npm pack`, extracts into `~/.must-b/extensions/<id>/`, and enables it in config.
...
- CLI: `must-b voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `must-b.extensions` points at the built entrypoint (`dist/index.js`).
