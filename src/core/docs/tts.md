...
Must-b can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere Must-b can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, Must-b defaults
...
TTS config lives under `messages.tts` in `must-b.json`.
...
- If `provider` is **unset**, Must-b prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.must-b/settings/tts.json`, override with `MUSTB_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, Must-b retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Must-b registers
