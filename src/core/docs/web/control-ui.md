...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/must-b`)
...
If the page fails to load, start the Gateway first: `must-b gateway`.
...
you revoke it with `must-b devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.must-b/must-b.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop must-b`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Must-b
...
Must-b **blocks** Control UI connections without device identity.
