...
```bash
must-b tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `must-b tui --deliver`
...
- Local shell commands receive `MUSTB_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `MUSTB_THEME=light` before launching `must-b tui`.
- To force the original dark palette instead, set `MUSTB_THEME=dark`.
...
- Check the Gateway logs: `must-b logs --follow`.
- Confirm the agent can run: `must-b status` and `must-b models status`.
...
- No agents in picker: check `must-b agents list` and your routing config.
