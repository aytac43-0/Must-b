...
summary: "`must-b configure` 的 CLI 参考（交互式配置提示）"
title: configure
x-i18n:
...
提示：不带子命令的 `must-b config` 会打开相同的向导。使用 `must-b config get|set|unset` 进行非交互式编辑。
...
```bash
must-b configure
must-b configure --section models --section channels
```
