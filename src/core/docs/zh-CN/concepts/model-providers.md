...
- CLI 辅助工具：`must-b onboard`、`must-b models list`、`must-b models set <provider/model>`。

## 内置提供商（pi-ai 目录）

Must-b 附带 pi-ai 目录。这些提供商**不需要** `models.providers` 配置；只需设置认证 + 选择模型。
...
- CLI：`must-b onboard --auth-choice token`（粘贴 setup-token）或 `must-b models auth paste-token --provider anthropic`
...
- CLI：`must-b onboard --auth-choice openai-codex` 或 `must-b models auth login --provider openai-codex`
...
- CLI：`must-b onboard --auth-choice opencode-zen`
...
- Antigravity OAuth 作为捆绑插件提供（`google-antigravity-auth`，默认禁用）。
  - 启用：`must-b plugins enable google-antigravity-auth`
  - 登录：`must-b models auth login --provider google-antigravity --set-default`
- Gemini CLI OAuth 作为捆绑插件提供（`google-gemini-cli-auth`，默认禁用）。
  - 启用：`must-b plugins enable google-gemini-cli-auth`
  - 登录：`must-b models auth login --provider google-gemini-cli --set-default`
  - 注意：你**不需要**将客户端 ID 或密钥粘贴到 `must-b.json` 中。CLI 登录流程将令牌存储在 Gateway 网关主机的认证配置文件中。
...
must-b models set opencode/claude-opus-4-5
must-b models list
```
