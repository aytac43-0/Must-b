...
Must-b 的 Gateway 网关暴露了一个简单的 HTTP 端点用于直接调用单个工具。它始终启用，但受 Gateway 网关认证和工具策略限制。
...
- `x-must-b-message-channel: <channel>`（示例：`slack`、`telegram`）
- `x-must-b-account-id: <accountId>`（当存在多个账户时）
