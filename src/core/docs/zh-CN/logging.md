...
你可以在 `~/.must-b/must-b.json` 中覆盖此设置：
...
所有日志配置都在 `~/.must-b/must-b.json` 的 `logging` 下。
...
- Must-b 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `must-b plugins enable diagnostics-otel` 启用插件。
...
- `must-b.tokens`（计数器，属性：`must-b.token`、`must-b.channel`、`must-b.provider`、`must-b.model`）
- `must-b.cost.usd`（计数器，属性：`must-b.channel`、`must-b.provider`、`must-b.model`）
- `must-b.run.duration_ms`（直方图，属性：`must-b.channel`、`must-b.provider`、`must-b.model`）
- `must-b.context.tokens`（直方图，属性：`must-b.context`、`must-b.channel`、`must-b.provider`、`must-b.model`）

消息流：

- `must-b.webhook.received`（计数器，属性：`must-b.channel`、`must-b.webhook`）
- `must-b.webhook.error`（计数器，属性：`must-b.channel`、`must-b.webhook`）
- `must-b.webhook.duration_ms`（直方图，属性：`must-b.channel`、`must-b.webhook`）
- `must-b.message.queued`（计数器，属性：`must-b.channel`、`must-b.source`）
- `must-b.message.processed`（计数器，属性：`must-b.channel`、`must-b.outcome`）
- `must-b.message.duration_ms`（直方图，属性：`must-b.channel`、`must-b.outcome`）

队列 + 会话：

- `must-b.queue.lane.enqueue`（计数器，属性：`must-b.lane`）
- `must-b.queue.lane.dequeue`（计数器，属性：`must-b.lane`）
- `must-b.queue.depth`（直方图，属性：`must-b.lane` 或 `must-b.channel=heartbeat`）
- `must-b.queue.wait_ms`（直方图，属性：`must-b.lane`）
- `must-b.session.state`（计数器，属性：`must-b.state`、`must-b.reason`）
- `must-b.session.stuck`（计数器，属性：`must-b.state`）
- `must-b.session.stuck_age_ms`（直方图，属性：`must-b.state`）
- `must-b.run.attempt`（计数器，属性：`must-b.attempt`）
...
- `must-b.webhook.error`
  - `must-b.channel`、`must-b.webhook`、`must-b.chatId`、`must-b.error`
- `must-b.message.processed`
  - `must-b.channel`、`must-b.outcome`、`must-b.chatId`、`must-b.messageId`、`must-b.sessionKey`、`must-b.sessionId`、`must-b.reason`
- `must-b.session.stuck`
  - `must-b.state`、`must-b.ageMs`、`must-b.queueDepth`、`must-b.sessionKey`、`must-b.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `must-b doctor`。
