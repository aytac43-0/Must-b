---
read_when:
  - 你想在 Must-b 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 Must-b 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
must-b plugins enable minimax-portal-auth  # 如果已加载则跳过
must-b gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/must-b/must-b/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `must-b models list` 和 `must-b models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `must-b configure` 并选择 **MiniMax M2.1**，或
