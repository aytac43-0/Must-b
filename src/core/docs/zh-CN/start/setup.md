...
summary: 设置指南：在保持最新的同时保持你的 Must-b 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.must-b/workspace`（工作区）+ `~/.must-b/must-b.json`（配置）。
...
- **配置：** `~/.must-b/must-b.json`（JSON/JSON5 格式）
- **工作区：** `~/.must-b/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm must-b setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **Must-b.app**（菜单栏）。
...
- 运行 `must-b setup`，然后 `must-b channels login`，然后手动启动 Gateway 网关（`must-b gateway`）。
...
- **配对允许列表**：`~/.must-b/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.must-b/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.must-b/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.must-b/workspace` 和 `~/.must-b/` 保持为"你的东西"；不要将个人提示/配置放入 `must-b` 仓库。
