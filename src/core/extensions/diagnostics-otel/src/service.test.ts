...
vi.mock("must-b/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("must-b/plugin-sdk/diagnostics-otel")>(
    "must-b/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
const OTEL_TEST_STATE_DIR = "/tmp/must-b-diagnostics-otel-test";
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("must-b.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("must-b.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("must-b.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("must-b.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("must-b.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("must-b.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("must-b.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("must-b.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("must-b.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("must-b.webhook.processed");
    expect(spanNames).toContain("must-b.message.processed");
    expect(spanNames).toContain("must-b.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["must-b.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("must-b.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "must-b.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["must-b.reason"]).toBe("string");
    expect(String(attrs?.["must-b.reason"])).not.toContain(
