...
import type {
  ...
  Must-bPluginService,
} from "must-b/plugin-sdk/diagnostics-otel";
import {
  onDiagnosticEvent,
  redactSensitiveText,
...
const DEFAULT_SERVICE_NAME = "must-b";
...
export function createDiagnosticsOtelService(): Must-bPluginService {
  ...
  return {
    ...
    async start(ctx) {
      ...
      const meter = metrics.getMeter("must-b");
      const tracer = trace.getTracer("must-b");
      ...
      const recordMessageProcessed = (
        ...
        if (evt.reason) {
          spanAttrs["must-b.reason"] = redactSensitiveText(evt.reason);
        }
      ...
      const recordSessionState = (
        ...
        const attrs: Record<string, string> = { "must-b.state": evt.state };
        if (evt.reason) {
          attrs["must-b.reason"] = redactSensitiveText(evt.reason);
        }
