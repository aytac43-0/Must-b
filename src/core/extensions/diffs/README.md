...
Read-only diff viewer plugin for **Must-b** agents.
...
Set plugin-wide defaults in `~/.must-b/must-b.json`:
...
Before:
Must-b supports plugins.

After:
Must-b supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/must-b-diffs`).
