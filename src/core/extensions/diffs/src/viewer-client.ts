...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-must-b-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-must-b-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.must-bDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.must-bDiffsError = "true";
