# Google Gemini CLI Auth (Must-b plugin)
...
```bash
must-b plugins enable google-gemini-cli-auth
```
...
```bash
must-b models auth login --provider google-gemini-cli --set-default
```
...
- `MUSTB_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `MUSTB_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
