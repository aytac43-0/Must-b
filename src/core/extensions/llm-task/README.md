...
`must-b.invoke --each`) without adding new Must-b code per workflow.
...
This extension depends on Must-b internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Must-b extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.must-b/extensions` as a standalone plugin directory.
