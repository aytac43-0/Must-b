...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["must-bPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["must-b", "pollId"]) ??
    readNestedString(value, ["must-b", "poll", "id"]) ??
    readNestedString(value, ["data", "must-bPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "must-b", "pollId"]);
  if (!pollId) {
    return null;
  }
