...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.MUSTB_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-nostr-"));
  process.env.MUSTB_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.MUSTB_STATE_DIR?.trim() || stateEnv.CLAWDBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".must-b");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.MUSTB_STATE_DIR = previous;
    }
