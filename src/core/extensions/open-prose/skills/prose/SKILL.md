...
metadata: { "must-b": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Must-b `sessions_spawn`
- **File I/O** == Must-b `read`/`write`
- **Remote fetch** == Must-b `web_fetch` (or `exec` with curl when POST is required)
