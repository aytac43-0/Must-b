...
import type {
  MustBPluginApi,
  MustBPluginCommandDefinition,
  PluginCommandContext,
} from "must-b/plugin-sdk/phone-control";
...
function createApi(params: {
  ...
  registerCommand: (command: MustBPluginCommandDefinition) => void;
}): MustBPluginApi {
  return {
    id: "phone-control",
    name: "phone-control",
...
describe("phone-control plugin", () => {
  it("arms sms.send as part of the writes group", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-phone-control-test-"));
    try {
      ...
      let command: MustBPluginCommandDefinition | undefined;
