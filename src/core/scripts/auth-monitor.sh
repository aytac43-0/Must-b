...
#
# Suggested cron: */30 * * * * /home/admin/must-b/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Must-b notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., must-b-alerts)
...
STATE_FILE="$HOME/.must-b/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Must-b if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use must-b
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Must-b to $NOTIFY_PHONE..."
            must-b send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Must-b is down. Run: ssh l36 '~/must-b/scripts/mobile-reauth.sh'" "urgent"
    exit 1
