...
import { discoverMustBPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']must-b\/plugin-sdk["']/;
...
function main() {
  const discovery = discoverMustBPlugins({});
  ...
  if (offenders.length > 0) {
    console.error("Bundled plugin source files must not import monolithic must-b/plugin-sdk.");
    for (const file of offenders.toSorted()) {
      const relative = path.relative(process.cwd(), file) || file;
      console.error(`- ${relative}`);
    }
    console.error(
      "Use must-b/plugin-sdk/<channel> for channel plugins, /core for startup surfaces, or /compat for broader internals.",
    );
    process.exit(1);
  }
