...
# Checks both Claude Code and Must-b auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
MUSTB_AUTH="$HOME/.must-b/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    must-b models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    must-b_status=$(check_must-b_auth 2>/dev/null || true)

    claude_expires=0
    must-b_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        must-b_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        must-b_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$MUSTB_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$must-b_status" \
        --arg be "$must-b_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            must-b: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$must-b_status" == EXPIRED* ]] || [[ "$must-b_status" == MISSING* ]]; then
        echo "MUSTB_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$must-b_status" == EXPIRING* ]]; then
        echo "MUSTB_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Must-b Auth (~/.must-b/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$MUSTB_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$MUSTB_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'must-b doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active must-b >/dev/null 2>&1; then
    echo -e "Must-b service: ${GREEN}running${NC}"
else
    echo -e "Must-b service: ${RED}NOT running${NC}"
fi
