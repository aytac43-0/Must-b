...
show_usage() {
    cat << EOF
clawlog - Must-b Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View Must-b logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Must-b logs flow through the macOS unified log (subsystem: ai.must-b).
