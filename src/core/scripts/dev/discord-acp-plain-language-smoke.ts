...
type DriverMode = "token" | "webhook" | "must-b";
...
function resolveStateDir(): string {
  const override = process.env.MUSTB_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.MUSTB_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".must-b");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver must-b] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Must-b spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|must-b> Driver transport mode (default: token)\n" +
    ...
    "  --must-b-bin <path>        Must-b CLI binary for driver=must-b (default: must-b)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  MUSTB_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  MUSTB_DISCORD_SMOKE_DRIVER\n" +
    "  MUSTB_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  MUSTB_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  MUSTB_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  MUSTB_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  MUSTB_DISCORD_SMOKE_AGENT\n" +
    "  MUSTB_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  MUSTB_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  MUSTB_DISCORD_SMOKE_POLL_MS\n" +
    "  MUSTB_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  MUSTB_DISCORD_SMOKE_MUSTB_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.MUSTB_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.MUSTB_DISCORD_SMOKE_DRIVER ||
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.MUSTB_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.MUSTB_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.MUSTB_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.MUSTB_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.MUSTB_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.MUSTB_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.MUSTB_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.MUSTB_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.MUSTB_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.MUSTB_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const must-bBin =
    resolveArg("--must-b-bin") || process.env.MUSTB_DISCORD_SMOKE_MUSTB_BIN || "must-b";
...
async function must-bCliJson<T>(params: { must-bBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.must-bBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`must-b ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesMustb(params: {
  ...
  const response = await must-bCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    must-bBin: params.must-bBin,
    args: [
      "message",
      "read",
      "--channel",
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `must-b-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await must-bCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("must-b message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Must-b.`,
        diagnostics: {
          bindingCandidates: [
            {
