...
INSTALL_URL="${MUSTB_INSTALL_URL:-https://must-b.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${MUSTB_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${MUSTB_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="must-b"
PACKAGE_NAME="${MUSTB_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (MUSTB_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${MUSTB_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${MUSTB_INSTALL_LATEST_OUT:-}"
fi
