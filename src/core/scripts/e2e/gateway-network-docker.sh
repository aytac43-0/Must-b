...
IMAGE_NAME="must-b-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="must-b-net-e2e-$$"
GW_NAME="must-b-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "MUSTB_GATEWAY_TOKEN=$TOKEN" \
  -e "MUSTB_SKIP_CHANNELS=1" \
  -e "MUSTB_SKIP_GMAIL_WATCHER=1" \
  -e "MUSTB_SKIP_CRON=1" \
  -e "MUSTB_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
