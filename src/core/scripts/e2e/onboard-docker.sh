...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  set -euo pipefail
	  ...
	  if [ -f dist/index.mjs ]; then
	    MUSTB_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    MUSTB_ENTRY="dist/index.js"
	  else
  ...
  export PATH="/tmp/must-b-bin:$PATH"
  mkdir -p /tmp/must-b-bin
  cat > /tmp/must-b-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$MUSTB_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_must-b_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/must-b-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/must-b-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$MUSTB_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/must-b-e2e-$1.XXXXXX"
  }

  set_isolated_must-b_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export MUSTB_HOME="$home_dir"
    export MUSTB_STATE_DIR="$home_dir/.must-b"
    export MUSTB_CONFIG_PATH="$MUSTB_STATE_DIR/must-b.json"
    mkdir -p "$MUSTB_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$MUSTB_STATE_DIR/workspace"
    config_path="$MUSTB_CONFIG_PATH"
    sessions_dir="$MUSTB_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_must-b_env "$home_dir"
	    ...
	    node "$MUSTB_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$MUSTB_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$MUSTB_ENTRY\" configure --section skills" send_skills_flow
