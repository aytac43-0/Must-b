...
# Usage: iwr -useb https://must-b.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://must-b.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\must-b",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Install-Must-bNpm {
    param([string]$Version = "latest")
    
    Write-Host "Installing Must-b (must-b@$Version)..." -Level info
    
    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g must-b@$Version --no-fund --no-audit 2>&1
        Write-Host "Must-b installed" -Level success
        return $true
    } catch {
...
function Install-Must-bGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing Must-b from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/must-b/must-b.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\must-b\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\must-b.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Must-b from git to $GitDir" -Level info
        } else {
            Install-Must-bGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Must-b via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-Must-bNpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'must-b onboard' to complete setup" -Level info
    }
