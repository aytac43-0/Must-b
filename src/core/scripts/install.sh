...
# Must-b Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://must-b.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Must-b."
...
GUM_VERSION="${MUSTB_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Must-b Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://must-b.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$MUSTB_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.must-b.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_must-b_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.must-b-* "$npm_root"/must-b 2>/dev/null || true
}

extract_must-b_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_must-b_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/must-b" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/must-b"|"/usr/local/bin/must-b")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/must-b/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale must-b symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing must-b binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Must-b package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_must-b_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Must-b npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*must-b" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_must-b_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_must-b_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Must-b npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an must-b binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "MUSTB_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "MUSTB_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "MUSTB_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "MUSTB_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "MUSTB_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "MUSTB_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "MUSTB_BETA" "CLAWDBOT_BETA"
map_legacy_env "MUSTB_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "MUSTB_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "MUSTB_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "MUSTB_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "MUSTB_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "MUSTB_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${MUSTB_TAGLINE_INDEX:-}" ]]; then
        if [[ "${MUSTB_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((MUSTB_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${MUSTB_NO_ONBOARD:-0}
NO_PROMPT=${MUSTB_NO_PROMPT:-0}
DRY_RUN=${MUSTB_DRY_RUN:-0}
INSTALL_METHOD=${MUSTB_INSTALL_METHOD:-}
MUSTB_VERSION=${MUSTB_VERSION:-latest}
USE_BETA=${MUSTB_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/must-b"
GIT_DIR=${MUSTB_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${MUSTB_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${MUSTB_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${MUSTB_VERBOSE:-0}"
MUSTB_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Must-b installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://must-b.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/must-b)
...
Environment variables:
  MUSTB_INSTALL_METHOD=git|npm
  MUSTB_VERSION=latest|next|<semver>
  MUSTB_BETA=0|1
  MUSTB_GIT_DIR=...
  MUSTB_GIT_UPDATE=0|1
  MUSTB_NO_PROMPT=1
  MUSTB_DRY_RUN=1
  MUSTB_NO_ONBOARD=1
  MUSTB_VERBOSE=1
  MUSTB_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://must-b.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://must-b.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://must-b.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Must-b checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Must-b source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_must-b_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"must-b"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://must-b.ai/install.sh | bash"
}
...
ensure_must-b_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/must-b" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/must-b" ]]; then
        ln -sf "$npm_root/must-b/dist/entry.js" "${npm_bin}/must-b"
        ui_info "Created must-b bin link at ${npm_bin}/must-b"
    fi
    return 0
}

# Check for existing Must-b installation
check_existing_must-b() {
    if [[ -n "$(type -P must-b 2>/dev/null || true)" ]]; then
        ui_info "Existing Must-b installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make must-b show as \"command not found\" in new terminals."
...
warn_must-b_not_found() {
    ui_warn "Installed, but must-b is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t must-b 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named must-b; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_must-b_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P must-b 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/must-b" ]]; then
        echo "${npm_bin}/must-b"
        return 0
    fi
...
install_must-b_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/must-b/must-b.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Must-b from git checkout: ${repo_dir}"
    else
        ui_info "Installing Must-b from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Must-b" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Must-b" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/must-b" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/must-b"
    ui_success "Must-b wrapper installed to \$HOME/.local/bin/must-b"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view must-b dist-tags.beta 2>/dev/null || true)"
...
install_must-b() {
    local package_name="must-b"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            MUSTB_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="must-b"
        else
            MUSTB_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${MUSTB_VERSION}" ]]; then
        MUSTB_VERSION="latest"
    fi

    local resolved_version=""
    resolved_version="$(npm view "${package_name}@${MUSTB_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Must-b v${resolved_version}"
    else
        ui_info "Installing Must-b (${MUSTB_VERSION})"
    fi
    local install_spec=""
    if [[ "${MUSTB_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${MUSTB_VERSION}"
    fi

    if ! install_must-b_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_must-b_paths
        install_must-b_npm "${install_spec}"
    fi

    if [[ "${MUSTB_VERSION}" == "latest" && "${package_name}" == "must-b" ]]; then
        if ! resolve_must-b_bin &> /dev/null; then
            ui_warn "npm install must-b@latest failed; retrying must-b@next"
            cleanup_npm_must-b_paths
            install_must-b_npm "must-b@next"
        fi
    fi

    ensure_must-b_bin_link || true

    ui_success "Must-b installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${MUSTB_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_must-b_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (must-b not on PATH yet)"
        warn_must-b_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${MUSTB_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.must-b/workspace-${profile}"
    else
        echo "${HOME}/.must-b/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${MUSTB_CONFIG_PATH:-$HOME/.must-b/must-b.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run must-b onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but must-b not on PATH; skipping onboarding"
        warn_must-b_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run must-b onboard to retry"
        return
    }
}
...
resolve_must-b_version() {
    local version=""
    local raw_version_output=""
    local claw="${MUSTB_BIN:-}"
    if [[ -z "$claw" ]] && command -v must-b &> /dev/null; then
        claw="$(command -v must-b)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_must-b_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/must-b/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/must-b/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    detected_checkout="$(detect_must-b_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Must-b checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set MUSTB_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        install_must-b_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/must-b" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/must-b"
    ...
    MUSTB_BIN="$(resolve_must-b_bin || true)"
    ...
    installed_version=$(resolve_must-b_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Must-b installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Must-b installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/must-b"
        ui_kv "Update command" "must-b update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://must-b.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running must-b doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                MUSTB_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                MUSTB_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                MUSTB_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run must-b doctor and must-b plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run must-b onboard later"
        else
            local config_path="${MUSTB_CONFIG_PATH:-$HOME/.must-b/must-b.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (must-b not on PATH yet)"
                    warn_must-b_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run must-b onboard to finish setup"
            return 0
        fi
    fi

    if command -v must-b &> /dev/null; then
        local claw="${MUSTB_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_must-b_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (must-b daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if MUSTB_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: must-b daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${MUSTB_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
