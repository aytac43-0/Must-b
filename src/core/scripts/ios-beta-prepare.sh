...
- regenerates apps/ios/Must-b.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${MUSTB_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid MUSTB_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid MUSTB_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid MUSTB_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid MUSTB_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing MUSTB_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(MUSTB_URL_SLASH)$(MUSTB_URL_SLASH)#g'
)"
...
MUSTB_CODE_SIGN_STYLE = Automatic
MUSTB_DEVELOPMENT_TEAM = ${TEAM_ID}
MUSTB_IOS_SELECTED_TEAM = ${TEAM_ID}
MUSTB_APP_BUNDLE_ID = ai.must-b.client
MUSTB_SHARE_BUNDLE_ID = ai.must-b.client.share
MUSTB_ACTIVITY_WIDGET_BUNDLE_ID = ai.must-b.client.activitywidget
MUSTB_WATCH_APP_BUNDLE_ID = ai.must-b.client.watchkitapp
MUSTB_WATCH_EXTENSION_BUNDLE_ID = ai.must-b.client.watchkitapp.extension
MUSTB_APP_PROFILE =
MUSTB_SHARE_PROFILE =
MUSTB_PUSH_TRANSPORT = relay
MUSTB_PUSH_DISTRIBUTION = official
MUSTB_URL_SLASH = /
MUSTB_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
MUSTB_PUSH_APNS_ENVIRONMENT = production
EOF
