...
ZIP=${1:?"Usage: $0 Must-b-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/must-b/must-b/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  ...
  if [[ "$ZIP_NAME" =~ ^Must-b-([0-9]+(\.[0-9]+){1,2}([-.][0-9A-Za-z]+([.-][0-9A-Za-z]+)*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/must-b/must-b/releases/download/v${VERSION}/"}
