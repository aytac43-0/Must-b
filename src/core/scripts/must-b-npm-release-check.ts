...
const EXPECTED_REPOSITORY_URL = "https://github.com/must-b/must-b";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "must-b") {
    errors.push(`package.json name must be "must-b"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.must-b !== "must-b.mjs") {
    errors.push(
      `package.json bin.must-b must be "must-b.mjs"; found "${pkg.bin?.must-b ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`must-b-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `must-b-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
