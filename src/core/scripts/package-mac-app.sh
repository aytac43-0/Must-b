...
# Build and bundle Must-b into a minimal .app we can open.
# Outputs to dist/Must-b.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Must-b.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Must-b"
BUNDLE_ID="${BUNDLE_ID:-ai.must-b.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/must-b/must-b/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Must-b/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :Must-bBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :Must-bGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Must-b"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Must-b"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Must-b"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Must-b" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Must-b/Resources/Must-b.icns" "$APP_ROOT/Contents/Resources/Must-b.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Must-b/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "⏹  Stopping any running Must-b"
killall -q Must-b 2>/dev/null || true
