#!/usr/bin/env bash
# Reset Must-b like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${MUSTB_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Must-b.app/Contents/MacOS/Must-b"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Must-b"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Must-b"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Must-b"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.must-b.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/must-b-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${MUSTB_RESTART_LOG:-/tmp/must-b-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${MUSTB_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.must-b/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  MUSTB_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node must-b.mjs daemon install --force --runtime node"
      log "  node must-b.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.must-b/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.must-b.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Must-b instances"
kill_all_must-b
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Must-b"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Must-b.app" ]]; then
    APP_BUNDLE="/Applications/Must-b.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Must-b.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Must-b.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Must-b.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set MUSTB_APP_BUNDLE to your installed Must-b.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node must-b.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node must-b.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".must-b", "must-b.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: Must-b is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.must-b.gateway.plist' | head -n 40 || true"
fi
