...
const shouldBuild = (deps) => {
  if (deps.env.MUSTB_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.MUSTB_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[must-b] ${message}\n`);
};

const runMust-b = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["must-b.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
