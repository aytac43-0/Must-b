...
BASE_IMAGE="${BASE_IMAGE:-must-b-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-must-b-sandbox-common:bookworm-slim}"
...
MUSTB_DOCKER_BUILD_USE_BUILDX="${MUSTB_DOCKER_BUILD_USE_BUILDX:-0}"
MUSTB_DOCKER_BUILD_CACHE_FROM="${MUSTB_DOCKER_BUILD_CACHE_FROM:-}"
MUSTB_DOCKER_BUILD_CACHE_TO="${MUSTB_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${MUSTB_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${MUSTB_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${MUSTB_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${MUSTB_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${MUSTB_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=must-b.sandbox=1)
NOTE
