#!/bin/bash
# Setup Must-b Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'must-b-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Must-b message: Send warning via Must-b itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/must-b-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/must-b-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/must-b-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now must-b-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status must-b-auth-monitor.timer"
echo ""
echo "Quick commands:"
