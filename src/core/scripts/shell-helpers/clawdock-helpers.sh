#!/usr/bin/env bash
# ClawDock - Docker helpers for Must-b
# Inspired by Simon Willison's "Running Must-b in Docker"
# https://til.simonwillison.net/llms/must-b-docker
#
# Installation:
...
# Common paths to check for Must-b
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/must-b"
  "${HOME}/workspace/must-b"
  "${HOME}/projects/must-b"
  "${HOME}/dev/must-b"
  "${HOME}/code/must-b"
  "${HOME}/src/must-b"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Must-b at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/must-b"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Must-b not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/must-b/must-b.git ~/must-b"
    echo "  cd ~/must-b && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^MUSTB_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d must-b-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart must-b-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f must-b-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec must-b-gateway \
    bash -c 'echo "alias must-b=\"./must-b.mjs\"" > /tmp/.bashrc_must-b && bash --rcfile /tmp/.bashrc_must-b'
}
...
clawdock-cli() {
  _clawdock_compose run --rm must-b-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build must-b-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "MUSTB_GATEWAY_TOKEN=$token" must-b-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" must-b-gateway \
    bash -c './must-b.mjs config set gateway.remote.token "$TOKEN" && ./must-b.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec must-b-gateway \
    bash -c "./must-b.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart must-b-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm must-b-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec must-b-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'must-b config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Must-b${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (must-b alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to must-b project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.must-b)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'must-b channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'must-b status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.must-b.ai${_CLR_RESET}"
  echo ""
}
