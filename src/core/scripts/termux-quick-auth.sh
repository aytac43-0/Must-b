...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/must-b/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Must-b Re-Auth" -c "After getting key, run: ssh $SERVER '~/must-b/scripts/mobile-reauth.sh'" --id must-b-auth
        ;;
    *)
