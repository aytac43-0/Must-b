 ...
 * `must-b update`. Use it to verify the completion installation flow
...
\`must-b update\`, \`must-b doctor\`, and \`must-b onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Must-b Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`must-b completion --install\` later to enable.`));
    return;
  }
