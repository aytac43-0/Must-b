---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Must-b deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Must-b cron scheduling for periodic checks, or version status checks on a machine running Must-b (laptop, workstation, Pi, VPS).
---

# Must-b Host Hardening

## Overview

Assess and harden the host running Must-b, then align it to a user-defined risk tolerance without breaking access. Use Must-b security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Must-b changes the host firewall, SSH, or OS updates; it does not.
...
5. Must-b gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Must-b can access sensitive data.
...
### 2) Run Must-b security audits (read-only)

As part of the default read-only checks, run `must-b security audit --deep`. Only offer alternatives if the user requests them:

1. `must-b security audit` (faster, non-probing)
2. `must-b security audit --json` (structured output)

Offer to apply Must-b safe defaults (numbered):

1. `must-b security audit --fix`

Be explicit that `--fix` only tightens Must-b defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Must-b version/update status (read-only)

As part of the default read-only checks, run `must-b update status`.
...
- Credential hygiene notes (location of Must-b creds, prefer disk encryption)
...
After Must-b install or first hardening pass, run at least one baseline audit and version check:

- `must-b security audit`
- `must-b security audit --deep`
- `must-b update status`

Ongoing monitoring is recommended. Use the Must-b cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `must-b cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `must-b update status`
...
Before creating, `must-b cron list` and match on exact `name`. If found, `must-b cron edit <id> ...`.
If not found, `must-b cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `must-b update status` (preferred for source checkouts and channels)
2. `npm view must-b version` (published npm version)

## Must-b command accuracy

Use only supported commands and flags:

- `must-b security audit [--deep] [--fix] [--json]`
- `must-b status` / `must-b status --deep`
- `must-b health --json`
- `must-b update status`
- `must-b cron add|list|runs|run`

Do not invent CLI flags or imply Must-b enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Must-b compaction:
