...
const createTempDir = () => tempDirs.make("must-b-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets MUSTB_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "must-b",
    });

    expect(env.MUSTB_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("must-b");
  });

  it("overrides pre-existing MUSTB_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      MUSTB_SHELL: "wrong",
    });
    expect(env.MUSTB_SHELL).toBe("acp-client");
  ...
  it("preserves MUSTB_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        MUSTB_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["MUSTB_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Must-b bridge", () => {
    ...
    expect(env.MUSTB_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.MUSTB_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "must-b",
        serverArgs: ["acp"],
        defaultServerCommand: "must-b",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("MUSTB_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "must-b", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "must-b", "dist", "entry.js");
    const shimPath = path.join(dir, "must-b.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\must-b\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/must-b-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/must-b-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/must-b-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
