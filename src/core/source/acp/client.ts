...
import { ensureMustBCliOnPath } from "../infra/path-env.js";
import {
...
export async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  ensureMustBCliOnPath();
  const serverArgs = buildServerArgs(opts);

  const entryPath = resolveSelfEntryPath();
  const defaultServerCommand = entryPath ? process.execPath : "must-b";
  ...
  await client.initialize({
    ...
    clientInfo: { name: "must-b-acp-client", version: "1.0.0" },
  });
