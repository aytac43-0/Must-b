...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: Must-bConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: Must-bConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: Must-bConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: Must-bConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: Must-bConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: Must-bConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/must-b");
  });

  it("uses MUSTB_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "must-b-home");
    vi.stubEnv("MUSTB_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as Must-bConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".must-b", "workspace"));
  });

  it("uses MUSTB_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "must-b-home");
    vi.stubEnv("MUSTB_HOME", home);
    // Clear state dir so it falls back to MUSTB_HOME
    vi.stubEnv("MUSTB_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as Must-bConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".must-b", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/must-b-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/must-b-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-agent-scope-"));
