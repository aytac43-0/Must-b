...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: MustBConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: MustBConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: MustBConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: MustBConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: MustBConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: MustBConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("uses MUSTB_HOME for default agent workspace", () => {
    ...
    const workspace = resolveAgentWorkspaceDir({} as MustBConfig, "main");
  ...
  it("uses MUSTB_HOME for default agentDir", () => {
    ...
    const agentDir = resolveAgentDir({} as MustBConfig, "main");
