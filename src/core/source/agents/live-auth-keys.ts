...
const GOOGLE_LIVE_SINGLE_KEY = "MUSTB_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "MUSTB_LIVE_ANTHROPIC_KEY",
    listVar: "MUSTB_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "MUSTB_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `MUSTB_LIVE_${base}_KEY`;
