...
const DIRECT_ENABLED = Boolean(process.env.MUSTB_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isTruthyEnvValue(process.env.MUSTB_LIVE_REQUIRE_PROFILE_KEYS);
...
describeLive("live models (profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.MUSTB_LIVE_MODELS?.trim();
      const useModern = rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseModelFilter(rawModels) : null;
      const allowNotFoundSkip = useModern;
      const providers = parseProviderFilter(process.env.MUSTB_LIVE_PROVIDERS);
      const perModelTimeoutMs = toInt(process.env.MUSTB_LIVE_MODEL_TIMEOUT_MS, 30_000);
      const maxModels = toInt(process.env.MUSTB_LIVE_MAX_MODELS, 0);
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[live-models] capped to ${selectedCandidates.length}/${candidates.length} via MUSTB_LIVE_MAX_MODELS=${maxModels}`,
        );
      }
