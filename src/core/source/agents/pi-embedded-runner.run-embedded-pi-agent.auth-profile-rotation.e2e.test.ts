...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): Must-bConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): Must-bConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): Must-bConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverride({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `must-b-auth-rotation-${Date.now()}.log`),
    });
