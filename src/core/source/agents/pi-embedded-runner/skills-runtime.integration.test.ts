...
const originalBundledDir = process.env.MUSTB_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("must-b-bundled-");
  const workspaceDir = await createTempDir("must-b-workspace-");
...
afterEach(async () => {
  process.env.MUSTB_BUNDLED_PLUGINS_DIR = originalBundledDir;
