...
const defaultTools = createMustBodingTools({ senderIsOwner: true });
...
describe("createMustBodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: MustBonfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createMustBodingTools({
    ...
    const anthropicTools = createMustBodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: MustBonfig = {
      tools: {
        exec: {
    ...
    const allowed = createMustBodingTools({
    ...
    const denied = createMustBodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createMustBodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
