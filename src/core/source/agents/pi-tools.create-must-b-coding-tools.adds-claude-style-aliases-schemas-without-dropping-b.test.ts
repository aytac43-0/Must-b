...
const defaultTools = createMust-bCodingTools({ senderIsOwner: true });
...
describe("createMust-bCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: Must-bConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createMust-bCodingTools({
    ...
    const anthropicTools = createMust-bCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: Must-bConfig = {
      tools: {
        exec: {
    ...
    const allowed = createMust-bCodingTools({
    ...
    const denied = createMust-bCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createMust-bCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
