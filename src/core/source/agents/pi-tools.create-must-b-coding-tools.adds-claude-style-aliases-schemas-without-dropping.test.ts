...
import { __testing, createMust-bCodingTools } from "./pi-tools.js";
import { createMust-bReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
...
describe("createMust-bCodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createMust-bCodingTools({ senderIsOwner: true }).find(
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-depth-policy-"));
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createMust-bCodingTools({
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "must-b-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createMust-bReadTool(
      baseRead as unknown as Parameters<typeof createMust-bReadTool>[0],
    );
