...
async function withOutsideHardlinkInMust-bTmp<T>(
  ...
  const hardlinkPath = path.join(params.mustBTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.mustBTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.mustBTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.mustBTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    {
      name: "absolute paths under preferred Must-b tmp root",
      media: path.join(mustBTmpDir, "image.png"),
      expected: path.join(mustBTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Must-b tmp root",
      media: pathToFileURL(path.join(mustBTmpDir, "photo.png")).href,
      expected: path.join(mustBTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Must-b tmp root",
      media: path.join(mustBTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(mustBTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(mustBTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside must-b tmp root",
      media: path.join(os.tmpdir(), "outside-must-b", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Must-b tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(mustBTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(mustBTmpDir, { recursive: true });
      const symlinkPath = path.join(mustBTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Must-b tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  it("rejects symlinked Must-b tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInMust-bTmp(
