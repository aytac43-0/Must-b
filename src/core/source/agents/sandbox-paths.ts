...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const mustBTmpDir = path.resolve(resolvePreferredMust-bTmpDir());
  if (!isPathInside(mustBTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: mustBTmpDir });
  return resolved;
}
