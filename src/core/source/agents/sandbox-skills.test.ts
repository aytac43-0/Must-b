...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "must-b-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.MUSTB_BUNDLED_SKILLS_DIR = bundledDir;
