...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "must-b.sandbox=1");
  args.push("--label", `must-b.sessionKey=${params.scopeKey}`);
  args.push("--label", `must-b.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `must-b.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markMust-bExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "must-b.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`must-b sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`must-b sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("must-b sandbox recreate --all");
}
