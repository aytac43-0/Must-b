...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["MUSTB_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns MUSTB_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-bundled-override-"));
    process.env.MUSTB_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.MUSTB_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "must-b" }));
