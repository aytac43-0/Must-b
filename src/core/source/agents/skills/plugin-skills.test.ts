    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "must-b.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "must-b.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "must-b.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("must-b-");
  const acpxRoot = await tempDirs.make("must-b-acpx-plugin-");
  const helperRoot = await tempDirs.make("must-b-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("must-b-");
  const pluginRoot = await tempDirs.make("must-b-plugin-");
  const outsideDir = await tempDirs.make("must-b-outside-");
