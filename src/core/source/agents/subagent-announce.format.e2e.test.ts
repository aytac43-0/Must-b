...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/must-b/must-b/issues/31298
    previousFastTestEnv = process.env.MUSTB_TEST_FAST;
  ...
  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.MUSTB_TEST_FAST;
      return;
    }
    process.env.MUSTB_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // MUSTB_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Must-b runtime context (internal):");
