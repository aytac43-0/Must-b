...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["MUSTB_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-subagent-"));
  ...
  it("uses isolated temp state when MUSTB_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "must-b-test-state"));
  });
});
