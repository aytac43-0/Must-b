...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("must-b-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("must-b-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("must-b-sbx-");
