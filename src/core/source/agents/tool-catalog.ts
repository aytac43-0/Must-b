...
function buildCoreToolGroupMap() {
  ...
  const must-bTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInMust-bGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:must-b": must-bTools,
