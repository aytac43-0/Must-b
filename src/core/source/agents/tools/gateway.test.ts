...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    must-b: process.env.MUSTB_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.must-b === undefined) {
      delete process.env.MUSTB_GATEWAY_TOKEN;
    } else {
      process.env.MUSTB_GATEWAY_TOKEN = envSnapshot.must-b;
    }
  ...
  it("uses MUSTB_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.MUSTB_GATEWAY_TOKEN = "local-env-token";
