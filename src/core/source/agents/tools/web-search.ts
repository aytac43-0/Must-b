...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  if (provider === "brave") {
    return {
      ...
      message: `web_search (brave) needs a Brave Search API key. Run \`${formatCliCommand("must-b configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
  ...
  return withTrustedWebSearchEndpoint(
    {
      ...
      init: {
        ...
        headers: {
          ...
          "HTTP-Referer": "https://must-b.ai",
          "X-Title": "Must-b Web Search",
        },
