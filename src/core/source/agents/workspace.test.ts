...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses MUSTB_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      MUSTB_HOME: "/srv/must-b-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/must-b-home"), ".must-b", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".must-b", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("must-b-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-workspace-hardlink-"));
    try {
