...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.MUSTB_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".must-b", `workspace-${profile}`);
  }
  return path.join(home, ".must-b", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".must-b";
