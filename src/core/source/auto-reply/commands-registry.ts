...
export function isCommandEnabled(cfg: MustBonfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: MustBonfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as MustBonfig),
...
export function getCommandDetection(_cfg?: MustBonfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: MustBonfig) {
