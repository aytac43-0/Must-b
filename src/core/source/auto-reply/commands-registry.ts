...
export function isCommandEnabled(cfg: Must-bConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: Must-bConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as Must-bConfig),
...
export function getCommandDetection(_cfg?: Must-bConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: Must-bConfig) {
