...
function setTempHomeEnv(home: string): void {
  ...
  process.env.MUSTB_STATE_DIR = join(home, ".must-b");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "must-b-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".must-b", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
