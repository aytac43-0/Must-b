...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop must-b!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@must-b_bot", { botUsername: "must-b_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@must-b_bot", { botUsername: "must-b_bot" })).toBe(true);
