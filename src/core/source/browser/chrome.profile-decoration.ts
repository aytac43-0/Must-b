...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".must-b-profile-decorated");
}
...
export function decorateMust-bProfile(
  ...
  const desiredName = opts?.name ?? DEFAULT_MUSTB_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MUSTB_BROWSER_COLOR).toUpperCase();
