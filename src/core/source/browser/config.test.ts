...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and orange color", () => {
    ...
    expect(profile?.name).toBe("must-b");
    expect(profile?.driver).toBe("must-b");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const must-b = resolveProfile(resolved, "must-b");
    expect(must-b?.driver).toBe("must-b");
    expect(must-b?.cdpPort).toBe(18800);
    expect(must-b?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from MUSTB_GATEWAY_PORT when unset", () => {
    withEnv({ MUSTB_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const must-b = resolveProfile(resolved, "must-b");
      expect(must-b?.cdpPort).toBe(19012);
      expect(must-b?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ MUSTB_GATEWAY_PORT: undefined }, () => {
      ...
      const must-b = resolveProfile(resolved, "must-b");
      expect(must-b?.cdpPort).toBe(19022);
      expect(must-b?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(must-b?.cdpPort).toBe(19000);
    expect(must-b?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "must-b");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        must-b: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("must-b");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to must-b profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps must-b default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("must-b");
    ...
    it("keeps must-b default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("must-b");
    ...
    it("keeps must-b default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
