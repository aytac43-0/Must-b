...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_MUSTB_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "must-b" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_MUSTB_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_MUSTB_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Must-b browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for must-b/must-b CDP).
...
export function resolveBrowserConfig(
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_MUSTB_BROWSER_PROFILE_NAME]
        ? DEFAULT_MUSTB_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "must-b";
