...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      "MUSTB_GATEWAY_TOKEN",
      "MUSTB_EXTENSION_RELAY_RECONNECT_GRACE_MS",
      "MUSTB_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS",
    ]);
    process.env.MUSTB_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
    delete process.env.MUSTB_EXTENSION_RELAY_RECONNECT_GRACE_MS;
    delete process.env.MUSTB_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS;
  });
  ...
  it("uses relay-scoped token only for known relay ports", async () => {
    ...
    expect(Object.keys(headers)).toContain("x-must-b-relay-token");
    expect(headers["x-must-b-relay-token"]).not.toBe(TEST_GATEWAY_TOKEN);
  });
  ...
  it("allows CORS preflight from chrome-extension origins", async () => {
    ...
    const res = await fetch(`${sharedUrl}/json/version`, {
      ...
      headers: {
        ...
        "Access-Control-Request-Headers": "x-must-b-relay-token",
      },
    });
  ...
  it("closes CDP clients after reconnect grace when extension stays disconnected", async () => {
    process.env.MUSTB_EXTENSION_RELAY_RECONNECT_GRACE_MS = "150";
  ...
  it("stops advertising websocket endpoint after reconnect grace expires", async () => {
    process.env.MUSTB_EXTENSION_RELAY_RECONNECT_GRACE_MS = "120";
  ...
  it("accepts /json endpoints with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(sharedUrl)["x-must-b-relay-token"];
    expect(token).toBeTruthy();
  ...
  it("accepts raw gateway token for relay auth compatibility", async () => {
    ...
    const versionRes = await fetch(`${sharedUrl}/json/version`, {
      headers: { "x-must-b-relay-token": TEST_GATEWAY_TOKEN },
    });
    expect(versionRes.status).toBe(200);
  ...
  it("reuses an already-bound relay port when another process owns it", async () => {
    ...
    const fakeRelay = createServer((req, res) => {
      if (req.url?.startsWith("/json/version")) {
        const header = req.headers["x-must-b-relay-token"];
        ...
        res.end(JSON.stringify({ Browser: "Must-b/extension-relay" }));
        return;
      }
  ...
  it(
    ...
    async () => {
      process.env.MUSTB_EXTENSION_RELAY_RECONNECT_GRACE_MS = "200";
  ...
  it(
    ...
    async () => {
      process.env.MUSTB_EXTENSION_RELAY_RECONNECT_GRACE_MS = "2000";
  ...
  it("does not swallow EADDRINUSE when occupied port is not an must-b relay", async () => {
    const port = await getFreePort();
