...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.MUSTB_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/must-b" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          must-b: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchMust-bChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.MUSTB_GATEWAY_PORT;
  process.env.MUSTB_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.MUSTB_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.MUSTB_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.MUSTB_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.MUSTB_GATEWAY_PASSWORD;
  } else {
    process.env.MUSTB_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
