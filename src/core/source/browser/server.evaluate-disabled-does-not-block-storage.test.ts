  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          must-b: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.MUSTB_GATEWAY_PORT;
    process.env.MUSTB_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.MUSTB_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.MUSTB_GATEWAY_PASSWORD;
