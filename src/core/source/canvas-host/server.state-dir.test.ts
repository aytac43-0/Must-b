...
describe("canvas host state dir defaults", () => {
  it("uses MUSTB_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("must-b-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Must-b Canvas");
      } finally {
        await handler.close();
      }
    });
