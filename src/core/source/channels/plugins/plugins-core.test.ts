...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@must-b/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-catalog-"));
  ...
  it("uses the provided env for external catalog path resolution", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-catalog-home-"));
        ...
        entries: [
          {
            ...
            must-b: {
              ...
              install: {
                npmSpec: "@must-b/env-demo-channel",
              },
            },
          },
        ],
    ...
    const ids = listChannelPluginCatalogEntries({
      env: {
        ...process.env,
        MUSTB_PLUGIN_CATALOG_PATHS: "~/catalog.json",
        HOME: home,
      },
    }).map((entry) => entry.id);
  ...
  it("uses the provided env for default catalog paths", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-catalog-state-"));
        ...
        entries: [
          {
            ...
            must-b: {
              ...
              install: {
                npmSpec: "@must-b/default-env-demo",
              },
            },
          },
        ],
