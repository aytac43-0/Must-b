    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["must-b channels list", "List configured channels and auth profiles."],
          ["must-b channels status --probe", "Run channel status checks and probes."],
          [
            "must-b channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["must-b channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.must-b.ai/cli/channels",
        )}\n`,
    );
