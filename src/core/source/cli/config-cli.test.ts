...
/**
 * Test for issue #6070:
 * `must-b config set/unset` must update snapshot.resolved (user config after $include/${ENV},
...
function makeInvalidSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/custom-must-b.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: {},
...
describe("config cli", () => {
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__MUSTB_REDACTED__");
    });
  });
  ...
  describe("config validate", () => {
    ...
    it("returns machine-readable JSON with --json for invalid config", async () => {
      ...
      expect(payload.path).toBe("/tmp/custom-must-b.json");
  ...
  describe("config file", () => {
    it("prints the active config file path", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("/tmp/must-b.json");
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
    });
    ...
    it("handles config file path with home directory", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("/home/user/.must-b/must-b.json");
    });
  });
});
