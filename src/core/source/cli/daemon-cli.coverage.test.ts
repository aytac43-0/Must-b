...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      MUSTB_GATEWAY_PORT: String(params.port),
      ...(params.token ? { MUSTB_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.MUSTB_STATE_DIR = "/tmp/must-b-cli-state";
    process.env.MUSTB_CONFIG_PATH = "/tmp/must-b-cli-state/must-b.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        MUSTB_PROFILE: "dev",
        MUSTB_STATE_DIR: "/tmp/must-b-daemon-state",
        MUSTB_CONFIG_PATH: "/tmp/must-b-daemon-state/must-b.json",
        MUSTB_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.must-b.gateway.plist",
    });
