...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("must-b-daemon-install-int-");
    configPath = path.join(tempHome, "must-b.json");
    process.env.HOME = tempHome;
    process.env.MUSTB_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.MUSTB_GATEWAY_TOKEN).toBeUndefined();
  });
});
