...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    MUSTB_STATE_DIR: "/tmp/must-b-daemon",
    MUSTB_CONFIG_PATH: "/tmp/must-b-daemon/must-b.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.MUSTB_STATE_DIR ?? "/tmp/must-b-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.MUSTB_CONFIG_PATH ?? `${stateDir}/must-b.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/must-b-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.MUSTB_STATE_DIR = "/tmp/must-b-cli";
    process.env.MUSTB_CONFIG_PATH = "/tmp/must-b-cli/must-b.json";
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.MUSTB_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
