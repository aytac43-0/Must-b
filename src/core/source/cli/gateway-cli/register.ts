      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["must-b gateway run", "Run the gateway in the foreground."],
            ["must-b gateway status", "Show service status and probe reachability."],
            ["must-b gateway discover", "Find local and wide-area gateway beacons."],
            ["must-b gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.must-b.ai/cli/gateway")}\n`,
      ),
  );
