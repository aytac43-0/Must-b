...
function warnInlinePasswordFlag() {
  defaultRuntime.error(
    "Warning: --password can be exposed via process listings. Prefer --password-file or MUSTB_GATEWAY_PASSWORD.",
  );
}
...
async function runGatewayCommand(opts: GatewayRunOpts) {
  const isDevProfile = process.env.MUSTB_PROFILE?.trim().toLowerCase() === "dev";
  ...
  if (opts.claudeCliLogs) {
    ...
    process.env.MUSTB_CLAUDE_CLI_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.MUSTB_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.MUSTB_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (process.env.MUSTB_SERVICE_MARKER?.trim()) {
  ...
  if (!opts.allowUnconfigured && mode !== "local") {
    if (!configExists) {
      defaultRuntime.error(
        `Missing config. Run \`${formatCliCommand("must-b setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
      );
    } else {
      defaultRuntime.error(
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or MUSTB_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (
    ...
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token/password (or MUSTB_GATEWAY_TOKEN/MUSTB_GATEWAY_PASSWORD) or pass --token/--password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  try {
    ...
    if (
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("must-b gateway stop")}`,
      );
      try {
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--token <token>",
      "Shared token required in connect.params.auth.token (default: MUSTB_GATEWAY_TOKEN env if set)",
    )
