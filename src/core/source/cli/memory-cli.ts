    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["must-b memory status", "Show index and provider status."],
          ["must-b memory status --deep", "Probe embedding provider readiness."],
          ["must-b memory index --force", "Force a full reindex."],
          ['must-b memory search "meeting notes"', "Quick search using positional query."],
          [
            'must-b memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["must-b memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.must-b.ai/cli/memory")}\n`,
    );
