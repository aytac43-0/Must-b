    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "must-b node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["must-b node status", "Check node host service status."],
          ["must-b node install", "Install the node host service."],
          ["must-b node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.must-b.ai/cli/node")}\n`,
    );
