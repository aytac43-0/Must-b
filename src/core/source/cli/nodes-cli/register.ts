    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["must-b nodes status", "List known nodes with live status."],
          ["must-b nodes pairing pending", "Show pending node pairing requests."],
          ['must-b nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["must-b nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.must-b.ai/cli/nodes")}\n`,
    );
