...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@must-b/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@must-b/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@must-b/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@must-b/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @must-b/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@must-b/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@must-b/plugin-alpha@1.2.3",
        installPath: "/tmp/must-b/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@must-b/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @must-b/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@must-b/plugin-alpha@latest",
      true,
      "/tmp/must-b/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@must-b/plugin-alpha@latest",
      installPath: "/tmp/must-b/extensions/alpha",
      version: "1.2.3",
