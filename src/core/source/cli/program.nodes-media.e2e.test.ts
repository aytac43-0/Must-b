...
describe("cli program (nodes media)", () => {
  ...
  it("runs nodes camera snap and prints two MEDIA paths", async () => {
    ...
    expect(mediaPaths[0]).toContain("must-b-camera-snap-");
    expect(mediaPaths[1]).toContain("must-b-camera-snap-");
  ...
  it("runs nodes camera clip and prints one MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /must-b-camera-clip-front-.*\.mp4$/,
    });
  });
  ...
  it("runs nodes canvas snapshot and prints MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /must-b-canvas-snapshot-.*\.png$/,
    });
  });
    ...
    it.each([
      {
        ...
        expectedPathPattern: /must-b-camera-snap-front-.*\.jpg$/,
      },
      {
