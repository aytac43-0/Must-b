...
${formatHelpExamples([
  ['must-b agent --to +15555550123 --message "status update"', "Start a new session."],
  ['must-b agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'must-b agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'must-b agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['must-b agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.must-b.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.must-b.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "must-b agents set-identity --identity-file ~/.must-b/workspace/IDENTITY.md --agent main",
