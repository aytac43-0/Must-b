...
describe("tryRouteCli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    originalDisableRouteFirst = process.env.MUSTB_DISABLE_ROUTE_FIRST;
    delete process.env.MUSTB_DISABLE_ROUTE_FIRST;
    vi.resetModules();
  ...
  afterEach(() => {
    if (originalDisableRouteFirst === undefined) {
      ...
      process.env.MUSTB_DISABLE_ROUTE_FIRST = originalDisableRouteFirst;
    }
  });

  it("passes suppressDoctorStdout=true for routed --json commands", async () => {
    await expect(tryRouteCli(["node", "must-b", "status", "--json"])).resolves.toBe(true);
  ...
  it("does not pass suppressDoctorStdout for routed non-json commands", async () => {
    await expect(tryRouteCli(["node", "must-b", "status"])).resolves.toBe(true);
  ...
  it("routes status when root options precede the command", async () => {
    await expect(tryRouteCli(["node", "must-b", "--log-level", "debug", "status"])).resolves.toBe(
      true,
    );
