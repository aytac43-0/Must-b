...
const SANDBOX_EXAMPLES = {
  main: [
    ["must-b sandbox list", "List all sandbox containers."],
    ["must-b sandbox list --browser", "List only browser containers."],
    ["must-b sandbox recreate --all", "Recreate all containers."],
    ["must-b sandbox recreate --session main", "Recreate a specific session."],
    ["must-b sandbox recreate --agent mybot", "Recreate agent containers."],
    ["must-b sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["must-b sandbox list", "List all sandbox containers."],
    ["must-b sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["must-b sandbox recreate --all", "Recreate all containers."],
    ["must-b sandbox recreate --session main", "Recreate a specific session."],
    ["must-b sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["must-b sandbox recreate --browser --all", "Recreate only browser containers."],
    ["must-b sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["must-b sandbox explain", "Show effective sandbox config."],
    ["must-b sandbox explain --session agent:main:main", "Explain a specific session."],
    ["must-b sandbox explain --agent work", "Explain an agent sandbox."],
    ["must-b sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.must-b.ai/cli/sandbox")}\n`,
    )
    .action(() => {
