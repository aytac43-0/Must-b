...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-bundled-skills-test-"));
    process.env.MUSTB_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
