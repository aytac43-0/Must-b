...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/must-b-update-tests";
  let fixtureCount = 0;
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("must-b-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    readPackageName.mockResolvedValue("must-b");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/must-b-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "must-b-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Must-b update status");
  });
  ...
  it.each([
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ["npm", "i", "-g", "must-b@latest", "--no-fund", "--no-audit", "--loglevel=error"],
      expect.any(Object),
    );
  });
  ...
  it("honors --tag override", async () => {
    ...
    expect(runGatewayUpdate).not.toHaveBeenCalled();
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ["npm", "i", "-g", "must-b@next", "--no-fund", "--no-audit", "--loglevel=error"],
      expect.any(Object),
    );
  });
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("must-b-update");
    const localAppData = createCaseDir("must-b-localappdata");
  ...
  it("uses MUSTB_UPDATE_PACKAGE_SPEC for package updates", async () => {
    const tempDir = createCaseDir("must-b-update");
    mockPackageInstallStatus(tempDir);
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      [
        ...
        "i",
        "-g",
        "http://10.211.55.2:8138/must-b-next.tgz",
        "--no-fund",
        "--no-audit",
        "--loglevel=error",
      ],
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("must-b-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ MUSTB_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.MUSTB_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("must-b-update-wizard");
    await withEnvAsync({ MUSTB_GIT_DIR: tempDir }, async () => {
      setTty(true);
