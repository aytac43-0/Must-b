...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Must-b and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["must-b update", "Update a source checkout (git)"],
        ["must-b update --channel beta", "Switch to beta channel (git + npm)"],
        ["must-b update --channel dev", "Switch to dev channel (git + npm)"],
        ["must-b update --tag beta", "One-off update to a dist-tag or version"],
        ["must-b update --dry-run", "Preview actions without changing anything"],
        ["must-b update --no-restart", "Update without restarting the service"],
        ["must-b update --json", "Output result as JSON"],
        ["must-b update --yes", "Non-interactive (accept downgrade prompts)"],
        ["must-b update wizard", "Interactive update wizard"],
        ["must-b --update", "Shorthand for must-b update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run must-b update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.must-b.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["must-b update status", "Show channel + version status."],
          ["must-b update status --json", "JSON output."],
          ["must-b update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.must-b.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
