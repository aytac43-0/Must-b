...
const STEP_LABELS: Record<string, string> = {
  ...
  "must-b doctor entry": "Checking doctor entrypoint",
  "must-b doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g must-b@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g must-b@latest --omit=optional");
  }
