...
const UPDATE_QUIPS = [
  ...
  "Fresh code, same Must-b. Miss me?",
  ...
  "Must-b has leveled up. Sharper mind, smarter answers.",
  ...
  "Upgraded! Aytaç fixed stuff. Blame him if it breaks.",
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("must-b completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
async function maybeRestartService(params: {
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.MUSTB_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.MUSTB_UPDATE_IN_PROGRESS;
        }
      }
      ...
      if (!params.opts.json && restartInitiated) {
        ...
        if (health.healthy) {
          ...
          defaultRuntime.log(
            theme.muted(
              `Run \`${replaceCliName(formatCliCommand("must-b gateway status --deep"), CLI_NAME)}\` for details.`,
            ),
          );
        }
        defaultRuntime.log("");
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("must-b gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("must-b doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("must-b gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("must-b gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (opts.dryRun) {
    ...
    if (switchToGit) {
      ...
      actions.push(`Run global package manager update with spec must-b@${tag}`);
    }
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Must-b..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Must-b install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("must-b doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("must-b gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g must-b@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g must-b@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
