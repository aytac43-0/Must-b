...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "MUSTB_GATEWAY_TOKEN",
          placeholder: "MUSTB_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like MUSTB_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Must-b will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Must-b trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.must-b.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
