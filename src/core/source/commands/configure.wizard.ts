...
async function runGatewayHealthCheck(params: {
  ...
  const token =
    process.env.MUSTB_GATEWAY_TOKEN ?? process.env.CLAWDBOT_GATEWAY_TOKEN ?? configuredToken;
  const password =
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from must-b.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Must-b update wizard" : "Must-b configure");
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("must-b doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
