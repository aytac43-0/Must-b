...
const SEARCH_TOOL = "https://docs.must-b.ai/mcp.SearchMust-b";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.must-b.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('must-b docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.must-b.ai/");
      runtime.log(`Search: ${formatCliCommand('must-b docs "your query"')}`);
    }
    return;
  }
