...
describe("resolveGatewayAuthTokenForService", () => {
  ...
  it("falls back to MUSTB_GATEWAY_TOKEN when SecretRef is unresolved", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
  ...
  it("falls back to MUSTB_GATEWAY_TOKEN when SecretRef resolves to empty", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
...
describe("shouldRequireGatewayTokenForInstall", () => {
  ...
  it("requires token in inferred mode when password env exists only in shell", async () => {
    await withEnvAsync(
      { [envVar("MUST-B", "GATEWAY", "PASSWORD")]: "password-from-env" },
      async () => {
