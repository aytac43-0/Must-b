...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.MUSTB_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("must-b gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
