...
vi.mock("../daemon/service-audit.js", () => ({
  ...
  readEmbeddedGatewayToken: (
    ...
    command?.environmentValueSources?.MUSTB_GATEWAY_TOKEN === "file"
      ? undefined
      : command?.environment?.MUSTB_GATEWAY_TOKEN?.trim() || undefined,
  SERVICE_AUDIT_CODES: {
...
function setupGatewayTokenRepairScenario() {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      MUSTB_GATEWAY_TOKEN: "stale-token",
    },
  });
    ...
    issues: [
      {
        ...
        message: "Gateway service MUSTB_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  ...
  it("uses MUSTB_GATEWAY_TOKEN when config token is missing", async () => {
  ...
  it("does not flag entrypoint mismatch when symlink and realpath match", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/Library/pnpm/global/5/node_modules/must-b/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/must-b@2026.3.12/node_modules/must-b/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
    ...
    fsMocks.realpath.mockImplementation(async (value: string) => {
      if (value.includes("/global/5/node_modules/must-b/")) {
        return value.replace(
          "/global/5/node_modules/must-b/",
          "/global/5/node_modules/.pnpm/must-b@2026.3.12/node_modules/must-b/",
        );
      }
      return value;
    });
  ...
  it("does not flag entrypoint mismatch when realpath fails but normalized absolute paths match", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/opt/must-b/../must-b/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
  ...
  it("still flags entrypoint mismatch when canonicalized paths differ", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/.nvm/versions/node/v22.0.0/lib/node_modules/must-b/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
    ...
    await withEnvAsync(
      ...
      async () => {
        mocks.readCommand.mockResolvedValue({
          programArguments: gatewayProgramArguments,
          environment: {
            MUSTB_GATEWAY_TOKEN: "env-file-token",
          },
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing Must-b gateway next.",
    );
  });
});
