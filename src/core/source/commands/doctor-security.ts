...
import type { Must-bConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: Must-bConfig): string[] {
...
export async function noteSecurityWarnings(cfg: Must-bConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("must-b security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.must-b/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("must-b approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.must-b.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("must-b configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("must-b config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("must-b doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("must-b config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('must-b config set session.dmScope "per-channel-peer"') +
