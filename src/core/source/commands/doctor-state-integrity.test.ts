...
type EnvSnapshot = {
  HOME?: string;
  MUSTB_HOME?: string;
  MUSTB_STATE_DIR?: string;
  MUSTB_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    MUSTB_HOME: process.env.MUSTB_HOME,
    MUSTB_STATE_DIR: process.env.MUSTB_STATE_DIR,
    MUSTB_OAUTH_DIR: process.env.MUSTB_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: Must-bConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: Must-bConfig) {
...
async function runStateIntegrityText(cfg: Must-bConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.MUSTB_HOME = tempHome;
    process.env.MUSTB_STATE_DIR = path.join(tempHome, ".must-b");
    delete process.env.MUSTB_OAUTH_DIR;
    fs.mkdirSync(process.env.MUSTB_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when MUSTB_OAUTH_DIR is explicitly configured", async () => {
    process.env.MUSTB_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: Must-bConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints must-b-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: Must-bConfig = {};
    ...
    expect(text).toMatch(/must-b sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/must-b sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /must-b sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
