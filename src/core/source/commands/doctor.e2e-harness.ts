...
beforeEach(() => {
  ...
  setStdinTty(true);
  originalStateDir = process.env.MUSTB_STATE_DIR;
  originalUpdateInProgress = process.env.MUSTB_UPDATE_IN_PROGRESS;
  process.env.MUSTB_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalUpdateInProgress === undefined) {
    ...
    process.env.MUSTB_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
