...
import { readConfigFileSnapshot, writeConfigFile, type Must-bConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("must-b config set gateway.auth.mode token")} or ${formatCliCommand("must-b config set gateway.auth.mode password")}.`,
  ].join(" ");
}
