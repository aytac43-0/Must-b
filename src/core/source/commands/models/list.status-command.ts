...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("must-b models auth setup-token")}\` or \`${formatCliCommand("must-b configure")}\`.`
          : `Run \`${formatCliCommand("must-b configure")}\` or set an API key env var.`;
