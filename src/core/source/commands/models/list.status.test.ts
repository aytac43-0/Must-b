...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/must-b-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/must-b-dev/must-b.json",
    }),
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/must-b-agent");
    }
  }
}
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/must-b-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/must-b-agent-custom");
