...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under MUSTB_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("must-b-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-oauth-sync-"));
    ...
    process.env.MUSTB_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-oauth-nosync-"));
    process.env.MUSTB_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside MUSTB_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-oauth-external-"));
    process.env.MUSTB_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* MUSTB_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to MUSTB_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("must-b-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
