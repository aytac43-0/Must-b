...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("must-b-onboard-");
  ...
  it("writes gateway token auth into config", async () => {
    await withStateDir("state-noninteractive-", async (stateDir) => {
      ...
      const workspace = path.join(stateDir, "must-b");
  ...
  it("uses MUSTB_GATEWAY_TOKEN when --gateway-token is omitted", async () => {
    await withStateDir("state-env-token-", async (stateDir) => {
      ...
      process.env.MUSTB_GATEWAY_TOKEN = envToken;
        ...
        await runNonInteractiveOnboarding(
          {
            ...
            gatewayBind: "loopback",
            gatewayAuth: "token",
            gatewayTokenRefEnv: "MUSTB_GATEWAY_TOKEN",
          },
          runtime,
        );
        ...
        runNonInteractiveOnboarding(
          {
            nonInteractive: true,
            mode: "local",
            workspace: path.join(stateDir, "must-b"),
            authChoice: "skip",
            skipSkills: true,
            skipHealth: false,
            installDaemon: false,
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      process.env.MUSTB_STATE_DIR = stateDir;
      process.env.MUSTB_CONFIG_PATH = path.join(stateDir, "must-b.json");
