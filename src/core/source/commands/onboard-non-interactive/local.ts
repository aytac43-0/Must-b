...
export async function runNonInteractiveOnboardingLocal(params: {
      ...
      const message = [
        ...
        !opts.installDaemon
          ? [
              ...
              `Fix: start \`${formatCliCommand("must-b gateway run")}\`, re-run with \`--install-daemon\`, or use \`--skip-health\`.`,
  ...
  if (!opts.json) {
    runtime.log(
      `Tip: run \`${formatCliCommand("must-b configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.must-b.ai/tools/web`,
    );
  }
}
