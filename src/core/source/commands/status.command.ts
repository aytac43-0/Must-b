...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByMust-b ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByMust-b ? "installed · " : "";
  ...
  runtime.log(theme.heading("Must-b status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`must-b devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("must-b devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("must-b devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("must-b security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("must-b security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.must-b.ai/faq");
  runtime.log("Troubleshooting: https://docs.must-b.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("must-b status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("must-b logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("must-b status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("must-b gateway probe")}`);
  }
}
