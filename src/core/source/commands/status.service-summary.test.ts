...
describe("readServiceStatusSummary", () => {
  it("marks Must-b-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["must-b", "gateway", "run"] })),
    ...
    expect(summary.managedByMust-b).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByMust-b).toBe(false);
