...
describe("config env vars", () => {
  ...
  it("loads ${VAR} substitutions from ~/.must-b/.env on repeated runtime loads", async () => {
    await withTempHome(async (_home) => {
      await withEnvOverride({ BRAVE_API_KEY: undefined }, async () => {
        const stateDir = process.env.MUSTB_STATE_DIR?.trim();
        if (!stateDir) {
          throw new Error("Expected MUSTB_STATE_DIR to be set by withTempHome");
        }
