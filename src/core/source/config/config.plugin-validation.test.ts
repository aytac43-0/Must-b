...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "must-b.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      MUSTB_STATE_DIR: path.join(suiteHome, ".must-b"),
      CLAWDBOT_STATE_DIR: undefined,
      MUSTB_PLUGIN_MANIFEST_CACHE_MS: "10000",
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-config-plugin-validation-"));
