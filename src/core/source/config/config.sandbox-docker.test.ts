...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("must-b-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "must-b-sandbox-browser-global" },
      agentBrowser: { network: "must-b-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("must-b-sandbox-browser-agent");
  });
