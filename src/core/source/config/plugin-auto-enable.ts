...
function isWhatsAppConfigured(cfg: MustBonfig): boolean {
...
function isGenericChannelConfigured(cfg: MustBonfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: MustBonfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: MustBonfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: MustBonfig, env: NodeJS.ProcessEnv): string[] {
...
function isPluginExplicitlyDisabled(cfg: MustBonfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: MustBonfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: MustBonfig, pluginId: string): MustBonfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
