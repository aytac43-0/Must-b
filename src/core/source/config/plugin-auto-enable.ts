...
function isWhatsAppConfigured(cfg: Must-bConfig): boolean {
...
function isGenericChannelConfigured(cfg: Must-bConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: Must-bConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: Must-bConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: Must-bConfig, env: NodeJS.ProcessEnv): string[] {
...
function isPluginExplicitlyDisabled(cfg: Must-bConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: Must-bConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: Must-bConfig, pluginId: string): Must-bConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
