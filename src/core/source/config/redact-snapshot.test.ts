...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.must-b/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/must-b/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/must-b/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/must-b/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/must-b/nickserv-password.txt");
