...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ MUSTB_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-must-b",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-must-b#general");
  });
  ...
  it("derives session transcripts dir from MUSTB_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { MUSTB_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when MUSTB_STATE_DIR differs from stored paths", () => {
