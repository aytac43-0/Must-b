...
const { makeStorePath } = createCronStoreHarness({ prefix: "must-b-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses MUSTB_HOME for tilde expansion", () => {
    vi.stubEnv("MUSTB_HOME", "/srv/must-b-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/must-b-home", "cron", "jobs.json"));
  });
});
