...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "must-b" | "clawdbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["must-b", "clawdbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["must-b_service_marker"];
  const kindKeys = ["must-b_service_kind"];
...
function isMust-bGatewaySystemdService(name: string, contents: string): boolean {
  ...
  if (!name.startsWith("must-b-gateway")) {
    return false;
  }
...
function isMust-bGatewayTaskName(name: string): boolean {
  ...
  return normalized === defaultName || normalized.startsWith("must-b gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    results.push({
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "must-b" || isLegacyLabel(label),
    });
  }
