...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `must-b-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      MUSTB_LAUNCHD_LABEL: `ai.must-b.launchd-int-${testId}`,
      MUSTB_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
