...
describe("resolveGatewayProgramArguments", () => {
  it("uses realpath-resolved dist entry when running via npx shim", async () => {
    const argv1 = path.resolve("/tmp/.npm/_npx/63c3/node_modules/.bin/must-b");
    const entryPath = path.resolve("/tmp/.npm/_npx/63c3/node_modules/must-b/dist/entry.js");
  ...
  it("prefers symlinked path over realpath for stable service config", async () => {
    // Simulates pnpm global install where node_modules/must-b is a symlink
    // to .pnpm/must-b@X.Y.Z/node_modules/must-b
    const symlinkPath = path.resolve(
      "/Users/test/Library/pnpm/global/5/node_modules/must-b/dist/entry.js",
    );
    const realpathResolved = path.resolve(
      "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/must-b@2026.1.21-2/node_modules/must-b/dist/entry.js",
    );
  ...
  it("falls back to node_modules package dist when .bin path is not resolved", async () => {
    const argv1 = path.resolve("/tmp/.npm/_npx/63c3/node_modules/.bin/must-b");
    const indexPath = path.resolve("/tmp/.npm/_npx/63c3/node_modules/must-b/dist/index.js");
    process.argv = ["node", argv1];
