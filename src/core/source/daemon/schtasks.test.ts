...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when MUSTB_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".must-b", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when MUSTB_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", MUSTB_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".must-b-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers MUSTB_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        MUSTB_PROFILE: "rescue",
        MUSTB_STATE_DIR: "C:\\State\\must-b",
      },
      expected: path.join("C:\\State\\must-b", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", MUSTB_PROFILE: "default" },
      expected: path.join("/home/test", ".must-b", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Must-b Gateway",
          "cd /d C:\\Projects\\must-b",
          "set NODE_ENV=production",
          "set MUSTB_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\must-b",
          environment: {
            NODE_ENV: "production",
            MUSTB_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\must-b\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\must-b\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Must-b Share\\node.exe" "\\\\fileserver\\Must-b Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Must-b Share\\node.exe",
            "\\\\fileserver\\Must-b Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from MUSTB_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ MUSTB_STATE_DIR: path.join(tmpDir, "custom-state") }),
