...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds MUSTB_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `must-b gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service MUSTB_GATEWAY_TOKEN does not match gateway.auth.token in must-b.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.MUSTB_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.MUSTB_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `must-b gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
