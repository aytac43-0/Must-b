...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.MUSTB_GATEWAY_PORT).toBe("18789");
    expect(env.MUSTB_GATEWAY_TOKEN).toBeUndefined();
    expect(env.MUSTB_SERVICE_MARKER).toBe("must-b");
    expect(env.MUSTB_SERVICE_KIND).toBe("gateway");
    expect(typeof env.MUSTB_SERVICE_VERSION).toBe("string");
    expect(env.MUSTB_SYSTEMD_UNIT).toBe("must-b-gateway.service");
    expect(env.MUSTB_WINDOWS_TASK_NAME).toBe("Must-b Gateway");
    if (process.platform === "darwin") {
      expect(env.MUSTB_LAUNCHD_LABEL).toBe("ai.must-b.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", MUSTB_PROFILE: "work" },
      port: 18789,
    });
    expect(env.MUSTB_SYSTEMD_UNIT).toBe("must-b-gateway-work.service");
    expect(env.MUSTB_WINDOWS_TASK_NAME).toBe("Must-b Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.MUSTB_LAUNCHD_LABEL).toBe("ai.must-b.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through MUSTB_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", MUSTB_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.MUSTB_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to MUSTB_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.MUSTB_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers MUSTB_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        MUSTB_GATEWAY_TOKEN: "must-b-token",
        CLAWDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.MUSTB_GATEWAY_TOKEN).toBe("must-b-token");
  });

  it("omits MUSTB_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        MUSTB_GATEWAY_TOKEN: "   ",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".must-b"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", MUSTB_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".must-b-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", MUSTB_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".must-b"));
  });

  it("uses MUSTB_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", MUSTB_STATE_DIR: "/var/lib/must-b" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/must-b"));
  });

  it("expands ~ in MUSTB_STATE_DIR", () => {
    const env = { HOME: "/Users/test", MUSTB_STATE_DIR: "~/must-b-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/must-b-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { MUSTB_STATE_DIR: "C:\\State\\must-b" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\must-b");
  });
});
