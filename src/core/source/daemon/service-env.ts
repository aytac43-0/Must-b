...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.MUSTB_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    MUSTB_PROFILE: profile,
    MUSTB_GATEWAY_PORT: String(port),
    MUSTB_LAUNCHD_LABEL: resolvedLaunchdLabel,
    MUSTB_SYSTEMD_UNIT: systemdUnit,
    MUSTB_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    MUSTB_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    MUSTB_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.MUSTB_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    MUSTB_GATEWAY_TOKEN: gatewayToken,
    MUSTB_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    MUSTB_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    MUSTB_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    MUSTB_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    MUSTB_LOG_PREFIX: "node",
    MUSTB_SERVICE_MARKER: NODE_SERVICE_MARKER,
    MUSTB_SERVICE_KIND: NODE_SERVICE_KIND,
    MUSTB_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    MUSTB_STATE_DIR: sharedEnv.stateDir,
    MUSTB_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.MUSTB_STATE_DIR;
  const configPath = env.MUSTB_CONFIG_PATH;
