  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/must-b",
      },
    },
    session: { store: "/tmp/must-b-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bmust-b\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "must-b: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "Must-b" },
          },
        },
        eventPatch: {
