...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn(() => "/tmp/must-b-discord-process-test-sessions.json"),
}));
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/must-b-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/must-b-discord-process-test-sessions.json" },
      },
    });
