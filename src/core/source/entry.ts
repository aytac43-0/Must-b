...
import { ensureMust-bExecMarkerOnProcess } from "./infra/must-b-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";
import { attachChildProcessBridge } from "./process/child-process-bridge.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "must-b.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "must-b.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  process.title = "must-b";
  ensureMust-bExecMarkerOnProcess();
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.MUSTB_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.MUSTB_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.MUSTB_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.MUSTB_NODE_OPTIONS_READY = "1";
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `Must-b ${VERSION} (${commit})` : `Must-b ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[must-b] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[must-b] ${parsed.error}`);
      process.exit(2);
    }
