...
const LIVE_ANDROID_NODE = isTruthyEnvValue(process.env.MUSTB_LIVE_ANDROID_NODE);
...
function resolveGatewayConnection() {
  ...
  const urlOverride = readString(process.env.MUSTB_ANDROID_GATEWAY_URL);
  const details = buildGatewayConnectionDetails({
    config: cfg,
    ...(urlOverride ? { url: urlOverride } : {}),
  });
  const tokenOverride = readString(process.env.MUSTB_ANDROID_GATEWAY_TOKEN);
  const passwordOverride = readString(process.env.MUSTB_ANDROID_GATEWAY_PASSWORD);
...
function selectTargetNode(nodes: NodeListNode[]): NodeListNode {
  const nodeIdOverride = readString(process.env.MUSTB_ANDROID_NODE_ID);
  if (nodeIdOverride) {
    const match = nodes.find((node) => node.nodeId === nodeIdOverride);
    if (!match) {
      throw new Error(`MUSTB_ANDROID_NODE_ID not found in node.list: ${nodeIdOverride}`);
    }
    return match;
  }

  const nodeNameOverride = readString(process.env.MUSTB_ANDROID_NODE_NAME)?.toLowerCase();
  if (nodeNameOverride) {
    const match = nodes.find(
      (node) => readString(node.displayName)?.toLowerCase() === nodeNameOverride,
    );
    if (!match) {
      throw new Error(`MUSTB_ANDROID_NODE_NAME not found in node.list: ${nodeNameOverride}`);
    }
    return match;
  }
    ...
    if (!target.connected || !target.paired) {
      ...
      throw new Error(
        [
          ...
          pendingHint,
          "precondition: open app, keep foreground, ensure pairing approved (`must-b nodes pending` / `must-b nodes approve <requestId>`)",
        ].join("\n"),
      );
    }
