...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.MUSTB_GATEWAY_URL) ??
  ...
  const allowPrivateWs = process.env.MUSTB_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set MUSTB_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: must-b doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__MUSTB_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
