...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("must-b doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with MUSTB_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with MUSTB_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.MUSTB_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://must-b-gateway.ai:18789",
      onConnectError,
    });
