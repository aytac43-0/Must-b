...
const { createMustBools } = await import("../agents/must-b-tools.js");
...
type SessionSendTool = ReturnType<typeof createMustBools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createMustBools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
