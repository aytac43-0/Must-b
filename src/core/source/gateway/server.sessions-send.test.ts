...
const { createMust-bTools } = await import("../agents/must-b-tools.js");
...
type SessionSendTool = ReturnType<typeof createMust-bTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createMust-bTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["MUSTB_GATEWAY_PORT", "MUSTB_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.MUSTB_GATEWAY_PORT = String(gatewayPort);
  process.env.MUSTB_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("MUSTB_CONFIG_PATH missing in gateway test environment");
      }
