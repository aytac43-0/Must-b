...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "MUSTB_STATE_DIR",
  "MUSTB_CONFIG_PATH",
  "MUSTB_SKIP_BROWSER_CONTROL_SERVER",
  "MUSTB_SKIP_GMAIL_WATCHER",
  "MUSTB_SKIP_CANVAS_HOST",
  "MUSTB_BUNDLED_PLUGINS_DIR",
  "MUSTB_SKIP_CHANNELS",
  "MUSTB_SKIP_PROVIDERS",
  "MUSTB_SKIP_CRON",
  "MUSTB_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.MUSTB_STATE_DIR = path.join(tempHome, ".must-b");
  delete process.env.MUSTB_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.MUSTB_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.MUSTB_SKIP_GMAIL_WATCHER = "1";
  process.env.MUSTB_SKIP_CANVAS_HOST = "1";
  process.env.MUSTB_SKIP_CHANNELS = "1";
  process.env.MUSTB_SKIP_PROVIDERS = "1";
  process.env.MUSTB_SKIP_CRON = "1";
  process.env.MUSTB_TEST_MINIMAL_GATEWAY = "1";
  process.env.MUSTB_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "must-b-test-no-bundled-extensions")
    : "must-b-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".must-b-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".must-b-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__must-bTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__must-bTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["MUSTB_GATEWAY_TOKEN"]);
  const prev = process.env.MUSTB_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.MUSTB_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.MUSTB_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.MUSTB_GATEWAY_PASSWORD;
