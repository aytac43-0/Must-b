...
import { createMust-bTools } from "../agents/must-b-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-must-b-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-must-b-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-must-b-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-must-b-thread-id")?.trim() || undefined;
  ...
  const allTools = createMust-bTools({
