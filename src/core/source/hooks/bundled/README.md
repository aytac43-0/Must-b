...
This directory contains hooks that ship with Must-b. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.must-b/must-b.json`:
...
2. Restart gateway: `pkill -9 -f 'must-b.*gateway' && pnpm must-b gateway`
3. Enable the hook: `must-b hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.must-b.ai/automation/hooks
