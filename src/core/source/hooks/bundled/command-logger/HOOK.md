...
homepage: https://docs.must-b.ai/automation/hooks#command-logger
metadata:
  {
    "must-b":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.must-b/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.must-b/logs/commands.log ~/.must-b/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/must-b`:
   ```
   /home/username/.must-b/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
