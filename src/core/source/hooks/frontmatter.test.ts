...
`;
    ...
    expect(parsed.must-b.emoji).toBe("💾");
    expect(parsed.must-b.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.must-b.emoji).toBe("📝");
    expect(parsed.must-b.events).toEqual(["command"]);
    expect(parsed.must-b.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.must-b.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"must-b": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"must-b": {"events": ["test"]}}');
  });
...
describe("resolveMust-bMetadata", () => {
  it("extracts must-b metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveMust-bMetadata(frontmatter);
  ...
  it("returns undefined when must-b key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        must-b: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Must-b" },
            { id: "npm", kind: "npm", package: "@must-b/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@must-b/hook");
  });
...
`;
    ...
    const must-b = resolveMust-bMetadata(frontmatter);
    expect(must-b).toBeDefined();
    expect(must-b?.emoji).toBe("💾");
    expect(must-b?.events).toEqual(["command:new", "command:reset"]);
    expect(must-b?.requires?.config).toEqual(["workspace.dir"]);
    expect(must-b?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const must-b = resolveMust-bMetadata(frontmatter);
    expect(must-b?.emoji).toBe("disk");
    expect(must-b?.events).toEqual(["command:new"]);
  });
});
