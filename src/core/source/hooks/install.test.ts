...
const fixtureRoot = path.join(os.tmpdir(), `must-b-hook-install-${randomUUID()}`);
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@must-b/test-hooks",
      version: "0.0.1",
      must-b: { hooks: params.hooks },
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'metadata: {"must-b":{"events":["command:new"]}}',
        "---",
        "",
        "# One Hook",
      ].join("\n"),
  ...
  it("rejects hook pack entries that traverse outside package directory", async () => {
    ...
    expectPathInstallFailureContains(result, "must-b.hooks entry escapes package directory");
  });
  ...
  it("rejects hook pack entries that escape via symlink", async () => {
    ...
    expectPathInstallFailureContains(
      result,
      "must-b.hooks entry resolves outside package directory",
    );
  });
});
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@must-b/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@must-b/test-hooks@0.0.1",
    });
  ...
  it("rejects bare npm specs that resolve to prerelease versions", async () => {
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@must-b/test-hooks@0.0.2-beta.1",
      name: "@must-b/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@must-b/test-hooks@beta"');
    }
  });
});
