...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): Must-bConfig {
  return {
...
describe("exec approval forwarder", () => {
  ...
  it("prefers turn-source routing over stale session last route", async () => {
    vi.useFakeTimers();
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-exec-approval-forwarder-test-"));
    try {
