...
describe("resolveEffectiveHomeDir", () => {
  it("prefers MUSTB_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands MUSTB_HOME when set to ~", () => {
    const env = {
      MUSTB_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for MUSTB_HOME", () => {
    const result = resolveRequiredHomeDir(
      { MUSTB_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when MUSTB_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ MUSTB_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { MUSTB_HOME: "/srv/must-b-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/must-b-home")}/x`);
  });
