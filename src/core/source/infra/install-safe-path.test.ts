...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-install-safe-"));
    try {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-install-safe-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-install-safe-outside-"));
      try {
