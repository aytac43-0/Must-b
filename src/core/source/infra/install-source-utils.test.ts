...
const TEMP_DIR_PREFIX = "must-b-install-source-utils-";
...
async function expectPackFallsBackToDetectedArchive(params: { stdout: string }) {
  const cwd = await createTempDir("must-b-install-source-utils-");
  const archivePath = path.join(cwd, "must-b-plugin-1.2.3.tgz");
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("must-b-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-must-b-archive.tgz");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "must-b-plugin@1.2.3",
          name: "must-b-plugin",
          version: "1.2.3",
          filename: "must-b-plugin-1.2.3.tgz",
    ...
    const result = await runPack("must-b-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "must-b-plugin",
        version: "1.2.3",
        resolvedSpec: "must-b-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "must-b-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "must-b-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\nmust-b-plugin-1.2.3.tgz\n",
    });
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr: "npm error code E404\nnpm error 404  '@must-b/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@must-b/whatsapp", cwd);

    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("Package not found on npm");
      expect(result.error).toContain("@must-b/whatsapp");
      expect(result.error).toContain("docs.must-b.ai/tools/plugin");
    }
  });
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("must-b-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "must-b-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        "npm notice creating package\n" +
        JSON.stringify([
          {
            id: "@must-b/plugin-demo@2.0.0",
            filename: "must-b-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@must-b/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "must-b-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@must-b/plugin-demo@2.0.0",
      },
    });
  });
