...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__must-b_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "must-b-root");
...
describe("resolveMust-bPackageRoot", () => {
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "must-b");
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "must-b");
  ...
  it("returns null for non-must-b package roots", async () => {
