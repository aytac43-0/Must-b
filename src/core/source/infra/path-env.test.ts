...
let ensureMust-bCliOnPath: typeof import("./path-env.js").ensureMust-bCliOnPath;

describe("ensureMust-bCliOnPath", () => {
  const envKeys = [
    "PATH",
    "MUSTB_PATH_BOOTSTRAPPED",
    "MUSTB_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureMust-bCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling must-b exists", () => {
    const tmp = abs("/tmp/must-b-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "must-b");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.MUSTB_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.MUSTB_PATH_BOOTSTRAPPED = "1";
    ensureMust-bCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/must-b-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "must-b");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/must-b-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "must-b");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "must-b");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/must-b-path/case-linuxbrew");
