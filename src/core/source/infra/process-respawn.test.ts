...
describe("restartGatewayProcessWithFreshPid", () => {
  it("returns disabled when MUSTB_NO_RESPAWN is set", () => {
    process.env.MUSTB_NO_RESPAWN = "1";
    const result = restartGatewayProcessWithFreshPid();
  ...
  it("returns supervised when launchd hints are present on macOS (no kickstart)", () => {
    ...
    setPlatform("darwin");
    process.env.LAUNCH_JOB_LABEL = "ai.must-b.gateway";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
  ...
  it("spawns detached child with current exec argv", () => {
    delete process.env.MUSTB_NO_RESPAWN;
    clearSupervisorHints();
    setPlatform("linux");
  ...
  it("returns supervised when MUSTB_LAUNCHD_LABEL is set (stock launchd plist)", () => {
    clearSupervisorHints();
    expectLaunchdSupervisedWithoutKickstart();
  });

  it("returns supervised when MUSTB_SYSTEMD_UNIT is set", () => {
    clearSupervisorHints();
    setPlatform("linux");
    process.env.MUSTB_SYSTEMD_UNIT = "must-b-gateway.service";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(spawnMock).not.toHaveBeenCalled();
  });

  it("returns supervised when Must-b gateway task markers are set on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
  ...
  it("ignores node task script hints for gateway restart detection on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.MUSTB_TASK_SCRIPT = "C:\\must-b\\node.cmd";
    process.env.MUSTB_TASK_SCRIPT_NAME = "node.cmd";
    process.env.MUSTB_SERVICE_MARKER = "must-b";
    process.env.MUSTB_SERVICE_KIND = "node";
