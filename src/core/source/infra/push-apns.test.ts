...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-push-apns-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("push APNs registration store", () => {
  it("stores and reloads node APNs registration", async () => {
    ...
    expect(loaded?.topic).toBe("ai.must-b.ios");
...
describe("push APNs env config", () => {
  ...
  it("resolves inline private key and unescapes newlines", async () => {
    const env = {
      MUSTB_APNS_TEAM_ID: "TEAM123",
      MUSTB_APNS_KEY_ID: "KEY123",
      MUSTB_APNS_PRIVATE_KEY_P8:
  ...
  it("returns an error when required APNs auth vars are missing", async () => {
    ...
    expect(resolved.error).toContain("MUSTB_APNS_TEAM_ID");
  });

  it("resolves APNs relay config from env", () => {
    const resolved = resolveApnsRelayConfigFromEnv({
      MUSTB_APNS_RELAY_BASE_URL: "https://relay.example.com",
      MUSTB_APNS_RELAY_TIMEOUT_MS: "2500",
    } as NodeJS.ProcessEnv);
  ...
  it("lets relay env overrides win over gateway config", () => {
    const resolved = resolveApnsRelayConfigFromEnv(
      {
        MUSTB_APNS_RELAY_BASE_URL: "https://relay-override.example.com",
        MUSTB_APNS_RELAY_TIMEOUT_MS: "3000",
      } as NodeJS.ProcessEnv,
      {
        push: {
          apns: {
  ...
  it("rejects insecure APNs relay http URLs by default", () => {
    const resolved = resolveApnsRelayConfigFromEnv({
      MUSTB_APNS_RELAY_BASE_URL: "http://relay.example.com",
    } as NodeJS.ProcessEnv);
    expect(resolved).toMatchObject({
      ok: false,
    });
    ...
    expect(resolved.error).toContain("MUSTB_APNS_RELAY_ALLOW_HTTP=true");
  });

  it("allows APNs relay http URLs only when explicitly enabled", () => {
    const resolved = resolveApnsRelayConfigFromEnv({
      MUSTB_APNS_RELAY_BASE_URL: "http://127.0.0.1:8787",
      MUSTB_APNS_RELAY_ALLOW_HTTP: "true",
    } as NodeJS.ProcessEnv);
    expect(resolved).toMatchObject({
      ok: true,
      value: {
  ...
  it("rejects APNs relay URLs with query, fragment, or userinfo components", () => {
    const withQuery = resolveApnsRelayConfigFromEnv({
      MUSTB_APNS_RELAY_BASE_URL: "https://relay.example.com/path?debug=1",
    } as NodeJS.ProcessEnv);
    ...
    const withUserinfo = resolveApnsRelayConfigFromEnv({
      MUSTB_APNS_RELAY_BASE_URL: "https://user:pass@relay.example.com/path",
    } as NodeJS.ProcessEnv);
