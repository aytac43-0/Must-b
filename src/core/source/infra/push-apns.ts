...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.MUSTB_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.MUSTB_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set MUSTB_APNS_TEAM_ID and MUSTB_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.MUSTB_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.MUSTB_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.MUSTB_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set MUSTB_APNS_PRIVATE_KEY_P8 or MUSTB_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading MUSTB_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
