...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses must-b-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "must-b-gateway" },
          { pid: process.pid, cmd: "must-b-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'must-b'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple must-b pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "must-b-gateway" },
          { pid: pid2, cmd: "must-b-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-must-b pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks must-b processes — non-must-b listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 1) {
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: stalePid, cmd: "must-b-gateway" }]),
            stderr: "",
          };
        }
    ...
    it("lsof status 1 with non-empty must-b stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty must-b stdout must not be treated as port-free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 2) {
          // status 1 + must-b pid in stdout — container-restricted lsof reports partial results
          return {
            error: null,
            status: 1,
            stdout: lsofOutput([{ pid: stalePid, cmd: "must-b-gateway" }]),
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'must-b'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("must-b")
      // inside the mid-loop flush: a non-must-b cmd between two entries must not
      // be pushed, but the following must-b entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-must-b entry first, then must-b entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid must-b entry must still be found.
      const stdout = `p0\ncmust-b-gateway\np${stalePid}\ncmust-b-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero must-b pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-must-b stdout (line 145)", () => {
    it("treats status 1 + non-must-b stdout as port-free (not an must-b process)", () => {
      // status 1 + non-empty stdout where no must-b pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill must-b pids) it is effectively free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        // status 1 + non-must-b output — should be treated as free:true for our purposes
        return {
          error: null,
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no must-b pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
