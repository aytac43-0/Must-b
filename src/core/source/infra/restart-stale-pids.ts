 ...
 * returning control to the caller (typically `triggerMust-bRestart` →
...
/**
 * Parse must-b gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("must-b")) {
 ...
 * Returns only PIDs that belong to must-b gateway processes (not the current process).
 */
