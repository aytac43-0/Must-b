...
export function triggerMust-bRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.MUSTB_SYSTEMD_UNIT,
      process.env.MUSTB_PROFILE,
    );
  ...
  const label =
    process.env.MUSTB_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.MUSTB_PROFILE);
