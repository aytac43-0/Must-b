...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.MUSTB_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Must-b. You can keep running the web gateway without it: \`${formatCliCommand("must-b gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
