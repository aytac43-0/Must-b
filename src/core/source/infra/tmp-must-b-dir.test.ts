...
describe("resolvePreferredMust-bTmpDir", () => {
  it("prefers /tmp/must-b when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_MUSTB_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/must-b when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_MUSTB_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/must-b when /tmp/must-b is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/must-b when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/must-b is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/must-b is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/must-b is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Must-b temp dir/);
  });
