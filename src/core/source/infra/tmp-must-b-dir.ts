...
export function resolvePreferredMust-bTmpDir(
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "must-b" : `must-b-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[must-b] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Must-b temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Must-b temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_MUSTB_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_MUSTB_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_MUSTB_TMP_DIR)) {
      return POSIX_MUSTB_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_MUSTB_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_MUSTB_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_MUSTB_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_MUSTB_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_MUSTB_TMP_DIR;
