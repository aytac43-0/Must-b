...
const PRIMARY_PACKAGE_NAME = "must-b";
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "Must-b", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.MUSTB_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.MUSTB_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
