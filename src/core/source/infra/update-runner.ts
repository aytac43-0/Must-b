...
const DEFAULT_PACKAGE_NAME = "must-b";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["must-b", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "must-b-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "must-b.mjs");
    ...
    const doctorStep = await runStep(
      step("must-b doctor", doctorArgv, gitRoot, { MUSTB_UPDATE_IN_PROGRESS: "1" }),
    );
