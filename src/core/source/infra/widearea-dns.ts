...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.MUSTB_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*must-b-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "must-b";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "must-b");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "must-b-gw");
  ...
  records.push(`_must-b-gw._tcp IN PTR ${instanceLabel}._must-b-gw._tcp`);
  records.push(`${instanceLabel}._must-b-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._must-b-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; must-b-content-hash: ${contentHash}\n${contentBody}`;
}
