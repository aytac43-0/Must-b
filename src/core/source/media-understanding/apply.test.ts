...
function createGroqAudioConfig(): MustBonfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): MustBonfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): MustBonfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: MustBonfig = { tools: { media: { audio: {} } } };
