...
const TEMP_MEDIA_PREFIX = "must-b-media-";
...
function createGroqAudioConfig(): Must-bConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): Must-bConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): Must-bConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: Must-bConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        MUSTB_AGENT_DIR: isolatedAgentDir,
