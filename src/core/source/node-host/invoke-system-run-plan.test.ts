...
function withFakeRuntimeBin<T>(params: { binName: string; run: () => T }): T {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), `must-b-${params.binName}-bin-`));
...
function withFakeRuntimeBins<T>(params: { binNames: string[]; run: () => T }): T {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-runtime-bins-"));
  const binDir = path.join(tmp, "bin");
  fs.mkdirSync(binDir, { recursive: true });
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  for (const testCase of cases) {
    it.runIf(process.platform !== "win32")(testCase.name, () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-approval-hardening-"));
      const oldPath = process.env.PATH;
  ...
  for (const runtimeCase of mutableOperandCases) {
    it(`captures mutable ${runtimeCase.name} operands in approval plans`, () => {
      ...
      withFakeRuntimeBins({
        binNames,
        run: () => {
          const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-approval-script-plan-"));
  ...
  it("rejects bun package script names that do not bind a concrete file", () => {
    withFakeRuntimeBin({
      binName: "bun",
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-bun-package-script-"));
        try {
          const prepared = buildSystemRunApprovalPlan({
            command: ["bun", "run", "dev"],
            cwd: tmp,
  ...
  it("rejects deno eval invocations that do not bind a concrete file", () => {
    withFakeRuntimeBin({
      binName: "deno",
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-deno-eval-"));
        try {
          const prepared = buildSystemRunApprovalPlan({
  ...
  it("rejects tsx eval invocations that do not bind a concrete file", () => {
    withFakeRuntimeBin({
      binName: "tsx",
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-tsx-eval-"));
        try {
          const prepared = buildSystemRunApprovalPlan({
  ...
  it("rejects node inline import operands that cannot be bound to one stable file", () => {
    withFakeRuntimeBin({
      binName: "node",
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-node-import-inline-"));
        try {
  ...
  it("rejects ruby require preloads that approval cannot bind completely", () => {
    withFakeRuntimeBin({
      binName: "ruby",
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-ruby-require-"));
        try {
  ...
  it("rejects ruby load-path flags that can redirect module resolution after approval", () => {
    withFakeRuntimeBin({
      binName: "ruby",
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-ruby-load-path-"));
        try {
          fs.writeFileSync(path.join(tmp, "safe.rb"), 'puts "SAFE"\n');
          const prepared = buildSystemRunApprovalPlan({
  ...
  it("rejects shell payloads that hide mutable interpreter scripts", () => {
    withFakeRuntimeBin({
      binName: "node",
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-inline-shell-node-"));
        try {
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "must-b-shell-option-value-"));
    try {
