...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `must-b-plugins-${randomUUID()}`);
  mkdirSafe(dir);
  tempDirs.push(dir);
  return dir;
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      must-b: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverMustBlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".must-b", "extensions");
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".must-b", "extensions");
    mkdirSafe(workspaceExt);
  ...
  it("normalizes bundled provider package ids to canonical plugin ids", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@must-b/ollama-provider",
      extensions: ["./src/index.ts"],
    });
    fs.writeFileSync(
      path.join(globalExt, "src", "index.ts"),
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@must-b/demo-plugin-dir",
      extensions: ["./index.js"],
    });
  ...
  it("blocks extension entries that escape package directory", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@must-b/escape-pack",
