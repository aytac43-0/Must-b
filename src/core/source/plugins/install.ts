...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing must-b.extensions; update the plugin package to include must-b.extensions (for example ["./dist/index.js"]). See https://docs.must-b.ai/help/troubleshooting#plugin-install-fails-with-missing-must-b-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_MUSTB_EXTENSIONS: "missing_must-b_extensions",
  EMPTY_MUSTB_EXTENSIONS: "empty_must-b_extensions",
...
function ensureMust-bExtensions(params: { manifest: PackageManifest }):
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_MUSTB_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json must-b.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_MUSTB_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  // Prefer the canonical `id` from must-b.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-must-b"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "must-b security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "must-b security audit --deep" after install.`,
    );
  }
