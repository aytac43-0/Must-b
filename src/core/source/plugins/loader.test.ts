...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "must-b-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.MUSTB_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "must-b.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
      ...
      JSON.stringify(
        {
          ...
          must-b: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function useNoBundledPlugins() {
  process.env.MUSTB_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.MUSTB_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.MUSTB_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadMustBlugins", () => {
  ...
  it("does not reuse cached bundled plugin registries across env changes", () => {
    ...
    const second = loadMustBlugins({
      ...
      env: {
        ...process.env,
        MUSTB_BUNDLED_PLUGINS_DIR: bundledB,
      },
    });
  ...
  it("does not reuse cached registries when env-resolved install paths change", () => {
    useNoBundledPlugins();
    const must-bHome = makeTempDir();
    const ignoredHome = makeTempDir();
    const stateDir = makeTempDir();
    const pluginDir = path.join(must-bHome, "plugins", "tracked-install-cache");
    mkdirSafe(pluginDir);
    const plugin = writePlugin({
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadMustBlugins({
      env: {
        ...
        HOME: homeDir,
        MUSTB_HOME: undefined,
        MUSTB_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers MUSTB_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const must-bHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "must-b-home-demo",
      dir: path.join(must-bHome, "plugins", "must-b-home-demo"),
      filename: "index.cjs",
    ...
    const registry = loadMustBlugins({
      ...
      config: {
        plugins: {
          allow: ["must-b-home-demo"],
          entries: {
            "must-b-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/must-b-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "must-b-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  it("skips importing bundled memory plugins that are disabled by memory slot", () => {
    ...
    fs.writeFileSync(
      path.join(memoryADir, "must-b.plugin.json"),
      JSON.stringify(
        {
          id: "memory-a",
          kind: "memory",
    ...
    fs.writeFileSync(
      path.join(memoryBDir, "must-b.plugin.json"),
      JSON.stringify(
        {
          id: "memory-b",
          kind: "memory",
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    const stateDir = makeTempDir();
    withEnv({ MUSTB_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
  ...
  it("does not auto-load workspace-discovered plugins unless explicitly trusted", () => {
    ...
    const workspaceExtDir = path.join(workspaceDir, ".must-b", "extensions", "workspace-helper");
  ...
  it("does not warn about missing provenance for env-resolved load paths", () => {
    ...
    const ignoredHome = makeTempDir();
    const stateDir = makeTempDir();
    const pluginDir = path.join(must-bHome, "plugins", "tracked-load-path");
    mkdirSafe(pluginDir);
    const plugin = writePlugin({
  ...
  it("does not warn about missing provenance for env-resolved install paths", () => {
    ...
    const ignoredHome = makeTempDir();
    const stateDir = makeTempDir();
    const pluginDir = path.join(must-bHome, "plugins", "tracked-install-path");
    mkdirSafe(pluginDir);
    const plugin = writePlugin({
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("must-b/plugin-sdk").emptyPluginConfigSchema)(),
  register() {},
};`,
    });
