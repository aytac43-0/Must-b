...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `must-b-manifest-registry-${randomUUID()}`);
  mkdirSafe(dir);
  tempDirs.push(dir);
  return dir;
}
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "must-b.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "must-b.plugin.json");
  const linkedManifest = path.join(rootDir, "must-b.plugin.json");
  ...
  it("does not reuse cached load-path manifests across env home changes", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: {
        ...
        MUSTB_STATE_DIR: path.join(homeA, ".state"),
      },
    });
    const second = loadPluginManifestRegistry({
      cache: true,
      ...
      env: {
        ...
        HOME: homeB,
        MUSTB_HOME: undefined,
        MUSTB_STATE_DIR: path.join(homeB, ".state"),
      },
    });
