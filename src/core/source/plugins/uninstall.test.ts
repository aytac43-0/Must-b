...
function createSinglePluginWithEmptySlotsConfig(): Must-bConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): Must-bConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "must-b-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-must-b-extensions/my-plugin",
      },
      extensionsDir,
    });
