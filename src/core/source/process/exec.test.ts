...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    ...
    expect(resolved.MUSTB_BASE_ENV).toBe("base");
    expect(resolved.MUSTB_TEST_ENV).toBe("ok");
    expect(resolved.MUSTB_TO_REMOVE).toBeUndefined();
    expect(resolved.MUSTB_CLI).toBe(MUSTB_CLI_ENV_VALUE);
  });
