...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: Must-bConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: Must-bConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/must-b-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<Must-bConfig["bindings"]>[number];
