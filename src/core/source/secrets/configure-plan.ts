...
export type ConfigureCandidate = {
  ...
  pathSegments: string[];
  label: string;
  configFile: "must-b.json" | "auth-profiles.json";
...
export function buildConfigureCandidatesForScope(params: {
  ...
  const must-bCandidates = discoverConfigSecretTargets(params.config)
    ...
    .map((entry) => {
      ...
      return {
        ...
        label: entry.path,
        configFile: "must-b.json" as const,
  ...
  return [...must-bCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "must-b secrets configure",
