...
const MUSTB_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
  (entry) => entry.configFile === "must-b.json",
);
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of MUSTB_COMPILED_SECRET_TARGETS) {
...
const MUSTB_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: MUSTB_COMPILED_SECRET_TARGETS,
    entriesById: MUSTB_TARGETS_BY_ID,
  });
