...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep MUSTB_STATE_DIR on a local-only volume and re-run "${formatCliCommand("must-b security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: MustBonfig): SecurityAuditFinding[] {
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer MUSTB_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const must-bGatewayToken =
    typeof env.MUSTB_GATEWAY_TOKEN === "string" && env.MUSTB_GATEWAY_TOKEN.trim()
      ? env.MUSTB_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-must-b-session-key for per-request session routing. ` +
...
export function collectSandboxDangerousConfigFindings(cfg: MustBonfig): SecurityAuditFinding[] {
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: must-b-sandbox-browser), " +
...
export function collectLikelyMultiUserSetupFindings(cfg: MustBonfig): SecurityAuditFinding[] {
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "Must-b's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway.",
    remediation:
