...
// Prefix for Must-b-generated action IDs to scope our handler
const MUSTB_ACTION_PREFIX = "must-b:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${MUSTB_ACTION_PREFIX}`);

  // Handle Must-b modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
