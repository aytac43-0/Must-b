...
describe("createTelegramBot", () => {
  ...
  it("routes compact model callbacks by inferring provider", async () => {
    ...
    const storePath = `/tmp/must-b-telegram-model-compact-${process.pid}-${Date.now()}.json`;
  ...
  it("resets overrides when selecting the configured default model", async () => {
    ...
    const storePath = `/tmp/must-b-telegram-model-default-${process.pid}-${Date.now()}.json`;
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "Must-b" },
        },
      },
      me: { id: 999, username: "must-b_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
