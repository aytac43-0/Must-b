...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("must-b-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.MUSTB_STATE_DIR).toBe(path.join(tempHome.home, ".must-b"));
    ...
    expect(process.env.MUSTB_STATE_DIR).toBe(previousStateDir);
