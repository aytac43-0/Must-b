...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.MUSTB_TTS_PREFS;
      process.env.MUSTB_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      const originalFetch = globalThis.fetch;
      const fetchMock = vi.fn(async () => ({
        ok: true,
      ...
      try {
        ...
        globalThis.fetch = originalFetch;
        process.env.MUSTB_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: Must-bConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
