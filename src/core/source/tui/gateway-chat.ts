...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set MUSTB_GATEWAY_TOKEN/MUSTB_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.MUSTB_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.MUSTB_GATEWAY_PASSWORD);
