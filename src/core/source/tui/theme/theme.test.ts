...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      MUSTB_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when MUSTB_THEME=light", async () => {
    const mod = await importThemeWithEnv({ MUSTB_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when MUSTB_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ MUSTB_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats MUSTB_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ MUSTB_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("MUSTB_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      MUSTB_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ MUSTB_THEME: "light" });
    const darkMod = await importThemeWithEnv({ MUSTB_THEME: "dark" });
