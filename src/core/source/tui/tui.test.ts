...
describe("resolveInitialTuiAgentId", () => {
  const cfg: MustBonfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/must-b" },
        { id: "ops", workspace: "/tmp/must-b/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/must-b/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run must-b devices list");
    expect(state.pairingHint).toContain("must-b devices list");
  });
