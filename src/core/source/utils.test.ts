...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("must-b-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("must-b-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("must-b-lid-a-", async (first) => {
      await withTempDir("must-b-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.must-b when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "must-b-config-dir-"));
    try {
  ...
  it("expands MUSTB_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/must-b-home",
      MUSTB_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/must-b-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers MUSTB_HOME over HOME", () => {
    vi.stubEnv("MUSTB_HOME", "/srv/must-b-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/must-b-home"));
...
describe("shortenHomePath", () => {
  it("uses $MUSTB_HOME prefix when MUSTB_HOME is set", () => {
    vi.stubEnv("MUSTB_HOME", "/srv/must-b-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/must-b-home")}/.must-b/must-b.json`)).toBe(
      "$MUSTB_HOME/.must-b/must-b.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $MUSTB_HOME replacement when MUSTB_HOME is set", () => {
    vi.stubEnv("MUSTB_HOME", "/srv/must-b-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/must-b-home")}/.must-b/must-b.json`),
    ).toBe("config: $MUSTB_HOME/.must-b/must-b.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/must-b")).toBe(path.resolve(os.homedir(), "must-b"));
  });
  ...
  it("prefers MUSTB_HOME for tilde expansion", () => {
    vi.stubEnv("MUSTB_HOME", "/srv/must-b-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/must-b")).toBe(path.resolve("/srv/must-b-home", "must-b"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/must-b", env)).toBe(path.resolve("/srv/must-b-home", "must-b"));
  });
