...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".must-b");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.MUSTB_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$MUSTB_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via MUSTB_STATE_DIR.
