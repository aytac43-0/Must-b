...
declare const __MUSTB_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "must-b";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["MUSTB_VERSION"],
      runtimeVersion,
      env["MUSTB_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Must-b version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __MUSTB_VERSION__ === "string" ? __MUSTB_VERSION__ : undefined,
  bundledVersion: process.env.MUSTB_BUNDLED_VERSION,
});
