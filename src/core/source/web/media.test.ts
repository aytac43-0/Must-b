...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub MUSTB_STATE_DIR.
    // Also keep it outside the Must-b temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["MUSTB_STATE_DIR"]);
    process.env.MUSTB_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "must-b-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows default Must-b state workspace and sandbox roots", async () => {
  ...
  it("rejects default Must-b state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
