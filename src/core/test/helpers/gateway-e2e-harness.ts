...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `must-b-e2e-${name}-`));
  const configDir = path.join(homeDir, ".must-b");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          MUSTB_SKIP_PROVIDERS: "1",
          MUSTB_SKIP_GMAIL_WATCHER: "1",
          MUSTB_SKIP_CRON: "1",
          MUSTB_SKIP_BROWSER_CONTROL_SERVER: "1",
          MUSTB_SKIP_CANVAS_HOST: "1",
          MUSTB_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
