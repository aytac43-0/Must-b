...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    must-bHome: process.env.MUSTB_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("MUSTB_HOME", snapshot.must-bHome);
  restoreKey("MUSTB_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked MUSTB_HOME.
  delete process.env.MUSTB_HOME;
  process.env.MUSTB_STATE_DIR = path.join(base, ".must-b");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "must-b-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".must-b", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
