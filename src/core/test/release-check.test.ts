        ...
        extensions: [
          {
            ...
            packageJson: {
              ...
              must-b: {
                install: { npmSpec: "@must-b/googlechat" },
                releaseChecks: {
          ...
          },
          {
            ...
            packageJson: {
              ...
              must-b: { install: { npmSpec: "@must-b/feishu" } },
            },
          },
        ],
      }),
...
describe("collectBundledExtensionManifestErrors", () => {
  it("flags invalid bundled extension install metadata", () => {
    expect(
      ...
      "bundled extension 'broken' manifest invalid | must-b.install.npmSpec must be a non-empty string",
    ]);
  });
  ...
  it("flags invalid release-check allowlist metadata", () => {
    expect(
      ...
      "bundled extension 'broken' manifest invalid | must-b.releaseChecks.rootDependencyMirrorAllowlist must contain only non-empty strings",
    ]);
  });
});
  ...
  it("flags nested node_modules leaking into npm pack output", () => {
    expect(
      collectForbiddenPackPaths([
        ...
        "node_modules/.bin/must-b",
      ]),
    ).toEqual(["extensions/tlon/node_modules/.bin/tlon", "node_modules/.bin/must-b"]);
  });
});
