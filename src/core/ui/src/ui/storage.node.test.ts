  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __MUSTB_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
    delete window.__MUSTB_CONTROL_UI_BASE_PATH__;
    return;
  }
  Object.defineProperty(window, "__MUSTB_CONTROL_UI_BASE_PATH__", {
    value,
    writable: true,
    configurable: true,
  });
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", async () => {
    ...
    setControlUiBasePath(" /must-b/ ");

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/must-b"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", async () => {
    setTestLocation({
      ...
      pathname: "/apps/must-b/chat",
    });

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/must-b"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", async () => {
    ...
    sessionStorage.setItem("must-b.control.token.v1", "legacy-session-token");
    localStorage.setItem(
      "must-b.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/must-b",
    ...
    expect(JSON.parse(localStorage.getItem("must-b.control.settings.v1") ?? "{}")).toEqual({
      gatewayUrl: "wss://gateway.example:8443/must-b",
      sessionKey: "agent",
  ...
  it("does not reuse a session token for a different gatewayUrl", async () => {
    ...
    localStorage.setItem(
      "must-b.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://other-gateway.example:8443/must-b",
        sessionKey: "main",
        lastActiveSessionKey: "main",
        theme: "claw",
        themeMode: "system",
